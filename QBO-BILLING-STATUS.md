...
- ✅ **Card processor = Stripe** (decided); same-day deposits via Stripe **Instant Payouts** with an **in-app button** (decided). Plan: **S1 Payments ledger ✅** → **S2** Payment Settings page (Stripe connection, accepted methods incl. ACH, QBO fee-account mapping, terms/surcharge, instant-payout button) → **S3** Stripe core (pay-by-link on invoices + `stripe-webhook` → gross **Payment** + exact **fee expense** from `balance_transaction` + **payout transfer**, all into QBO via a "Stripe clearing" account) → **S4** refunds/disputes. **Needs from Moroni:** a Stripe account + API keys (test mode ok) in Cloudflare env, the webhook registered in Stripe, and two QBO accounts ("Stripe clearing" bank + "Merchant Fees" expense). UPR must be the ONLY writer to QBO (don't also run a Stripe→QBO connector).
