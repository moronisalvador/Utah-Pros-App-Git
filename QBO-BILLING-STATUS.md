...
**Jun 25 2026 — Estimate builder (built, DORMANT behind `page:estimates`, off by default):** full line-item **estimate** tool mirroring invoices, QBO-synced, with convert-to-invoice. Migration `20260625_estimate_builder.sql` (applied): `estimate_line_items` table + QBO/builder columns on `estimates` + `recompute_estimate_from_lines` trigger + `generate_estimate_number` + RPCs `create_estimate_for_job` (multiple per job), `get_estimates`, `convert_estimate_to_invoice`. Worker `qbo-estimate.js` + `lib/quickbooks.js` `createEstimate/updateEstimate/deleteEstimate/sendEstimate`. **Convert links in QBO both ways:** UPR convert pushes the invoice with `LinkedTxn→Estimate` (`qbo-invoice.js`); and QBO's deposit-driven estimate→invoice auto-conversion mirrors into UPR via `qbo-payment-sync.js` `adoptInvoiceFromQboEstimate` (activates with the §4B QBO Payment webhook). Frontend: `EstimateEditor` (`/estimates/:id`), `Estimates` list (`/estimates`), `NewEstimateModal`, shared `AutoGrowTextarea` (multi-line scope-of-work descriptions, also adopted by InvoiceEditor); both editors widened. Nav + routes gated by the **`page:estimates`** flag (seeded disabled). Full detail in `UPR-Web-Context.md` → "QuickBooks Online — Estimates". **Owner activation:** flip `page:estimates` on in Dev Tools; grant non-admins the `estimates` nav key as needed. Needs a live QBO test (estimate push + convert), like the invoice flow.
