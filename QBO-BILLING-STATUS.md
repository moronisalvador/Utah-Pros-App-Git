...
- **Invoice numbers now match QBO (owner-requested):** UPR captures QBO's DocNumber on push (`invoices.qbo_doc_number`) and displays it, so the number you see in UPR == the one in QuickBooks. Job#/claim#/UPR-ref are also in the QBO invoice PrivateNote for reverse lookup. Note: the few invoices synced *before* this change (if any) get their `qbo_doc_number` the next time they're sent/updated — until then UPR falls back to `INV-######`.
