...
- **Full invoice builder** (commits `ea5ba87`, `8374dea`): invoices are built from **line items** — per line an **Item + Class** (dropdowns pulled live from QuickBooks via `qbo-query`, mirroring the QBO catalog), description, qty × rate. Total rolls up via the `recompute_invoice_from_lines()` trigger; push to QBO is **itemized** (each line → SalesItemLine w/ ItemRef + ClassRef). Builder-only (lump-sum removed). New `invoice_line_items.qbo_*` columns. Now on a **dedicated editor page** `/invoices/:id` (`InvoiceEditor`, commit `df80eb8`) — guarded build → explicit Send/Update to QuickBooks + Remove/Delete-draft (confirms); the claim A/R panel is read-only and opens the editor via Create/Edit invoice.
- **Payments ledger** (commit `e23e9fb`): Collections is now a 2-tab hub — **A/R · Outstanding** + **Payments** (cash-in history: date / client / claim·job / invoice / method·source / amount / QB-sync, with KPIs + search). New `get_payments_ledger()` RPC. This is **Stripe step S1**.
...
| **A/R views (redesign)** | **DONE — coherent 2-level system:** global `/collections` dashboard → per-claim **A/R workspace** (`ClaimCollectionPage` rebuilt, `c99a704`), plus the invoice panel on the claim page + client profile. All invoice-centric & consistent (`df8effb`, `098a25e`, `24f8834`, `c99a704`); orphaned `ARPage` cluster + stale docs removed (`dbf9a9f`). **Remaining (optional):** drop deprecated DB objects `billing_overview` / `get_ar_jobs` (harmless if left). | |
...
- ✅ **Card processor = Stripe** (decided); same-day deposits via Stripe **Instant Payouts** with an **in-app button** (decided). Plan: **S1 Payments ledger ✅** → **S2 Payment Settings ✅** (`8c8b0a7` — `/payments/settings`: accept card/ACH, terms, surcharge, QBO fee-account mapping; Stripe connect + instant-payout button present but inert until keys) → **S3** Stripe core (pay-by-link on invoices + `stripe-webhook` → gross **Payment** + exact **fee expense** from `balance_transaction` + **payout transfer**, all into QBO via a "Stripe clearing" account) → **S4** refunds/disputes. **Needs from Moroni:** a Stripe account + API keys (test mode ok) in Cloudflare env, the webhook registered in Stripe, and two QBO accounts ("Stripe clearing" bank + "Merchant Fees" expense). UPR must be the ONLY writer to QBO (don't also run a Stripe→QBO connector).
