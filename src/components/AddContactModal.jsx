          ...
          <div className="lookup-select-dropdown">
            {fil.length===0?(
              search.trim()
                ...
                :<div className="lookup-select-empty">No options found</div>
            ):(
              <>
                {fil.map(c=>(
                  <button key={c.id||c[nameKey]} className={`lookup-select-item${c[nameKey]===value?' active':''}`} onClick={()=>sel(c[nameKey])}>
                    <span className="lookup-select-name">{c[nameKey]}</span>
                    {(c.short_name||c.category)&&<span className="lookup-select-tag">{c.short_name||c.category}</span>}
                  </button>
                ))}
                {search.trim()&&!fil.find(c=>(c[nameKey]||'').toLowerCase()===search.toLowerCase())&&(
                  <button className="lookup-select-item lookup-select-custom" onClick={()=>sel(search.trim())}>Use "<strong>{search.trim()}</strong>"</button>
                )}
              </>
            )}
          </div>
...
export default function AddContactModal({onClose,onSave,carriers,referralSources}){
  ...
  const handleSave=async()=>{
    ...
    try{
      ...
      const data={name:form.name.trim(),phone,role:form.role,email:form.email?.trim()||null,company:form.company?.trim()||null,referral_source:form.referral_source?.trim()||null,tags:JSON.stringify(tags),notes:form.notes?.trim()||null,opt_in_status:false};
