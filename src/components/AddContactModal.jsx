...
// Export CarrierSelect for reuse in ContactProfile edit form
export { CarrierSelect };

/* ═══ ADD CONTACT MODAL ═══ */
export default function AddContactModal({onClose,onSave,carriers}){
  ...
  const Field=({label,field,type='text',placeholder,required})=>(<div className="form-group" style={{flex:1,marginBottom:0}}><label className="label">{label}{required&&' *'}</label>{type==='textarea'?<textarea className="input textarea" value={form[field]||''} onChange={e=>set(field,e.target.value)} rows={2} placeholder={placeholder}/>:type==='checkbox'?<label style={{display:'flex',alignItems:'center',gap:'var(--space-2)',cursor:'pointer',fontSize:'var(--text-sm)'}}><input type="checkbox" checked={form[field]||false} onChange={e=>set(field,e.target.checked)} style={{width:16,height:16}}/>{placeholder||label}</label>:<input ref={field==='name'?nameRef:undefined} className="input" type={type} value={form[field]||''} onChange={e=>set(field,e.target.value)} placeholder={placeholder}/> }</div>);
  const Select=({label,field,options})=>(<div className="form-group" style={{flex:1,marginBottom:0}}><label className="label">{label}</label><select className="input" value={form[field]||''} onChange={e=>set(field,e.target.value)} style={{cursor:'pointer'}}>{options.map(r=><option key={r.value} value={r.value}>{r.label}</option>)}</select></div>);
        ...
        {step==='pick'?(
          ...
          <>
            <div className="add-contact-body">
              ...
              <div className="add-contact-row"><Select label="Preferred Contact" field="preferred_contact_method" options={CONTACT_METHOD_OPTIONS}/><Select label="Language" field="preferred_language" options={LANG_OPTIONS}/></div>
              {role==='homeowner'&&(<><div className="cp-edit-section-label">Billing Address</div><div className="add-contact-row"><Field label="Street" field="billing_address" placeholder="1422 E Maple Ridge Dr"/></div><div className="add-contact-row"><Field label="City" field="billing_city" placeholder="Lehi"/><Field label="State" field="billing_state" placeholder="UT"/><Field label="ZIP" field="billing_zip" placeholder="84043"/></div><div className="cp-edit-section-label">Insurance</div><div className="add-contact-row"><CarrierSelect value={form.insurance_carrier} onChange={v=>set('insurance_carrier',v)} carriers={carriers}/><Field label="Policy #" field="policy_number" placeholder="SF-8820114"/></div></>)}
              {role==='adjuster'&&(<><div className="cp-edit-section-label">Adjuster Details</div><div className="add-contact-row"><CarrierSelect value={form.insurance_carrier} onChange={v=>set('insurance_carrier',v)} carriers={carriers}/></div><div className="add-contact-row"><Field label="Desk Phone" field="desk_phone" type="tel" placeholder="(800) 555-0100"/><Field label="Extension" field="desk_extension" placeholder="4412"/></div><div className="add-contact-row"><Field label="Territory / Region" field="territory" placeholder="Northern Utah - Salt Lake, Davis, Weber"/></div><Field label="Relationship Notes" field="relationship_notes" type="textarea" placeholder="Response time, negotiation style, preferences..."/></>)}
              {role==='vendor'&&(<><div className="cp-edit-section-label">Vendor Details</div><div className="add-contact-row"><Field label="Trade / Specialty" field="trade_specialty" placeholder="Flooring, plumbing, electrical..."/><Select label="Payment Terms" field="payment_terms" options={PAYMENT_TERMS}/></div><div className="add-contact-row"><Field label="W-9 on File" field="w9_on_file" type="checkbox" placeholder="W-9 received and on file"/></div></>)}
              {role==='subcontractor'&&(<><div className="cp-edit-section-label">Subcontractor Details</div><div className="add-contact-row"><Field label="Trade / Specialty" field="trade_specialty" placeholder="Drywall, painting, tile..."/><Select label="Payment Terms" field="payment_terms" options={PAYMENT_TERMS}/></div><div className="add-contact-row"><Field label="COI Expiration" field="coi_expiration" type="date"/><Field label="W-9 on File" field="w9_on_file" type="checkbox" placeholder="W-9 received and on file"/></div></>)}
