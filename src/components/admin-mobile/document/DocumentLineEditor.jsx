...
export default function DocumentLineEditor({ line, items, classes, busy, saveLabel, onPatch, onSave }) {
  ...
  return (
    <form className="am-invline-editor" onSubmit={(event) => event.preventDefault()}>
      ...
      <label className="am-invline-field"><span className="am-invline-label">Description</span><input className="am-invline-input am-invline-input--description" value={line.description || ''} onChange={(event) => onPatch({ description: event.target.value })} disabled={busy} /></label>
      ...
      <div className="am-invline-numbers">
        <label className="am-invline-field am-invline-field--quantity"><span className="am-invline-label">Quantity</span><input className="am-invline-input am-invline-input--number" inputMode="decimal" type="number" step="any" value={line.quantity ?? ''} onChange={(event) => onPatch({ quantity: event.target.value })} disabled={busy} /></label>
        <label className="am-invline-field am-invline-field--rate"><span className="am-invline-label">Rate</span><input className="am-invline-input am-invline-input--number" inputMode="decimal" type="number" step="any" value={line.unit_price ?? ''} onChange={(event) => onPatch({ unit_price: event.target.value })} disabled={busy} /></label>
      </div>
