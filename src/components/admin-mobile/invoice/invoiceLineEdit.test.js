...
describe('invoice line edit contract', () => {
  ...
  it('keeps finite negative adjustments valid and rejects non-numeric amounts', () => {
    expect(invoiceLineValidationError({ description: 'Credit', quantity: 1, unit_price: -25 })).toBe('');
    expect(invoiceLineValidationError({ description: 'Repair', quantity: 'not-a-number', unit_price: 25 }))
      .toBe('Enter a valid quantity.');
    expect(invoiceLineValidationError({ description: 'Repair', quantity: 1, unit_price: 'not-a-number' }))
  ...
  it('uses one idempotent human QBO helper with a safe line patch and no browser line RPC', () => {
    ...
    expect(source).toMatch(/const authHeaders = await getAuthHeader\(\);[\s\S]*if \(!isCurrentRoute\(\)\) return;[\s\S]*callQboInvoiceWorker\(\{[\s\S]*ownerId: user\?\.id,[\s\S]*authHeaders,[\s\S]*line_change: lineChange/);
