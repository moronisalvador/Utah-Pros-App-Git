    ...
    <div style={{
      ...
      {phone ? (
        <a href={`tel:${phone}`} style={{ ...btnBase, ...enabledStyle, textDecoration: 'none' }}>
          ...
          <span style={{ fontSize: 11, fontWeight: 600 }}>{t('actionBar.call')}</span>
        </a>
      ) : (
      ...
      {address ? (
        <button onClick={() => openMap(address)} style={{ ...btnBase, ...enabledStyle, background: 'none', border: 'none' }}>
          ...
          <span style={{ fontSize: 11, fontWeight: 600 }}>{t('actionBar.navigate')}</span>
        </button>
      ) : (
        <button disabled style={{ ...btnBase, ...disabledStyle, background: 'none', border: 'none' }}>
          <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
      ...
      {onDocuments && (
        <button onClick={onDocuments} style={{ ...btnBase, ...enabledStyle, background: 'none', border: 'none' }}>
          ...
          <span style={{ fontSize: 11, fontWeight: 600 }}>{t('actionBar.documents')}</span>
        </button>
      )}
    </div>
  );
