...
// QuickBooks. Create draft → set amount → Push to QuickBooks. Relocatable later.
export default function ClaimBilling({ jobs, db, canEdit }) {
  ...
  const saveAmount = async (inv) => {
    const amt = Number(amounts[inv.id]);
    if (!(amt >= 0)) { toast('Enter a valid amount', 'error'); return; }
    setBusy(inv.id);
    try {
      await db.update('invoices', `id=eq.${inv.id}`, { subtotal: amt, total: amt });
      setAmounts(a => { const n = { ...a }; delete n[inv.id]; return n; });
      await load();
      toast('Amount saved');
    } catch (e) { toast('Failed to save amount', 'error'); }
    finally { setBusy(null); }
  };

  const callWorker = async (inv, body, okMsg) => {
    setBusy(inv.id);
    try {
        ...
        return (
          <div key={job.id} style={{ border: '1px solid var(--border-light)', borderRadius: 'var(--radius-md)', padding: '10px 12px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 8, flexWrap: 'wrap' }}>
              ...
              {synced ? (
                <span style={{ fontSize: 11, fontWeight: 600, padding: '2px 8px', borderRadius: 'var(--radius-full)', background: '#f0fdf4', color: '#16a34a', border: '1px solid #bbf7d0' }}>
                  QuickBooks #{inv.qbo_invoice_id}
                </span>
              ) : inv?.qbo_sync_error ? (
                <span title={inv.qbo_sync_error} style={{ fontSize: 11, fontWeight: 600, padding: '2px 8px', borderRadius: 'var(--radius-full)', background: '#fef2f2', color: '#dc2626', border: '1px solid #fecaca', cursor: 'help' }}>
                  Error
                </span>
              ) : null}
            </div>
                ...
                {inv && !synced && (
                  <>
                    <input
                      ...
                      value={amounts[inv.id] ?? ''} onChange={e => setAmounts(a => ({ ...a, [inv.id]: e.target.value }))}
                    ...
                    />
                    <button className="btn btn-secondary btn-sm" disabled={busy === inv.id || amounts[inv.id] == null || amounts[inv.id] === ''} onClick={() => saveAmount(inv)}>
                      Save amount
                    </button>
                    <button className="btn btn-primary btn-sm" disabled={busy === inv.id} onClick={() => callWorker(inv, {}, d => `Pushed to QuickBooks (#${d.qbo_invoice_id})`)}>
                      {busy === inv.id ? 'Pushing…' : 'Push to QuickBooks'}
                    </button>
                  </>
                )}
                {synced && (
                  <button className="btn btn-secondary btn-sm" disabled={busy === inv.id} onClick={() => callWorker(inv, { action: 'delete' }, 'Removed from QuickBooks')}>
                    {busy === inv.id ? '…' : 'Remove from QuickBooks'}
                  </button>
                )}
              </div>
            )}
