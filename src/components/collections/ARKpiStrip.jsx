...
export default function ARKpiStrip({ kpis, onFilter, activeFilter }) {
  return (
    <div className="ar-kpi-strip">
      <div
        className={`ar-kpi-card ar-kpi-clickable${kpis.outstanding > 0 ? ' ar-kpi-alert' : ''}${activeFilter === 'needs_attention' ? ' ar-kpi-active' : ''}`}
        onClick={() => onFilter(activeFilter === 'needs_attention' ? 'all' : 'needs_attention')}
      >
      ...
      <div
        className={`ar-kpi-card ar-kpi-clickable${activeFilter === 'all' ? ' ar-kpi-active' : ''}`}
      ...
      <div
        className={`ar-kpi-card ar-kpi-clickable${activeFilter === 'paid' ? ' ar-kpi-active' : ''}`}
        onClick={() => onFilter(activeFilter === 'paid' ? 'all' : 'paid')}
      >
      ...
      <div
        className={`ar-kpi-card ar-kpi-clickable ar-kpi-desktop-only${activeFilter === 'needs_attention' ? ' ar-kpi-active' : ''}`}
        onClick={() => onFilter(activeFilter === 'needs_attention' ? 'all' : 'needs_attention')}
      >
