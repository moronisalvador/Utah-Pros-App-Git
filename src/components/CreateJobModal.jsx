 ...
 *   onClose() — close modal
 *   onCreated(result) — called after job created, result = { job, contact, claim_id, claim_number }
 *   prefillContact — optional contact object to pre-select (e.g. when opened from CustomerPage)
 */
export default function CreateJobModal({db,onClose,onCreated,prefillContact}){
  ...
  const onSearchChange=e=>{const v=e.target.value;setSearch(v);clearTimeout(timer.current);
    if(v.trim().length>=2)timer.current=setTimeout(()=>doSearch(v),300);else{setResults([]);setShowDrop(false);}};

  const selectContact=c=>{setContact(c);setSearch('');setShowDrop(false);
    if(c.billing_address||c.billing_city){setAddress(c.billing_address||'');setCity(c.billing_city||'');setState(c.billing_state||'UT');setZip(c.billing_zip||'');}};
  ...
  const fmtPh=phone=>{if(!phone)return'';const d=phone.replace(/\D/g,'');const n=d.startsWith('1')?d.slice(1):d;
    if(n.length===10)return`(${n.slice(0,3)}) ${n.slice(3,6)}-${n.slice(6)}`;return phone;};
  ...
  const handleSubmit=async()=>{
    ...
    try{
      const result=await db.rpc('create_job_with_contact',{
        ...
        p_division:division,p_source:'insurance',p_priority:3,
        p_type_of_loss:typeOfLoss||null,p_date_of_loss:dateOfLoss||null,
        p_address:address||null,p_city:city||null,p_state:state||null,p_zip:zip||null,
        p_insurance_company:insuranceCompany||null,p_claim_number:claimNumber||null,
          ...
          <div style={{marginBottom:'var(--space-3)'}}>
            ...
            {!contact?(
              <div ref={searchRef} style={{position:'relative'}}>
                ...
                {showDrop&&<div style={{position:'absolute',left:0,right:0,top:'100%',zIndex:20,background:'var(--bg-primary)',border:'1px solid var(--border-color)',borderRadius:'var(--radius-md)',boxShadow:'var(--shadow-lg)',maxHeight:240,overflowY:'auto',marginTop:4}}>
                  {results.length===0?(
                    <div style={{padding:'var(--space-3)',fontSize:13,color:'var(--text-tertiary)'}}>
                      {search.trim().length>=2?<>No clients found. <button style={{color:'var(--brand-primary)',background:'none',border:'none',cursor:'pointer',textDecoration:'underline',fontFamily:'inherit',fontSize:13}} onClick={()=>setShowAddContact(true)}>Create new</button></>:'Type 2+ characters'}
                    </div>
                  ):results.map(c=>(
                    <button key={c.id} onClick={()=>selectContact(c)} style={{display:'flex',alignItems:'center',gap:'var(--space-3)',width:'100%',padding:'var(--space-2) var(--space-3)',border:'none',background:'none',cursor:'pointer',textAlign:'left',fontFamily:'var(--font-sans)',borderBottom:'1px solid var(--border-light)'}}>
              ...
              <div style={{display:'flex',alignItems:'center',gap:'var(--space-3)',padding:'var(--space-2) var(--space-3)',background:'var(--bg-secondary)',borderRadius:'var(--radius-md)',border:'1px solid var(--border-light)'}}>
                ...
                <button className="btn btn-ghost btn-sm" onClick={clearContact} style={{width:26,height:26,padding:0}}><IconX style={{width:13,height:13}}/></button>
              </div>
            )}
          </div>
