...
const CM=[{value:'sms',label:'SMS'},{value:'call',label:'Phone Call'},{value:'email',label:'Email'}];
const DE={water:'\u{1F4A7}',mold:'\u{1F9A0}',reconstruction:'\u{1F3D7}\uFE0F'};
...
const fd=(v)=>{if(!v)return'\u2014';return new Date(v).toLocaleDateString('en-US',{month:'short',day:'numeric',year:'numeric'});};
const fdt=(v)=>{if(!v)return'\u2014';return new Date(v).toLocaleString('en-US',{month:'short',day:'numeric',hour:'numeric',minute:'2-digit'});};
const fm=(v)=>{if(v==null)return'\u2014';return`$${Number(v).toLocaleString('en-US',{minimumFractionDigits:2,maximumFractionDigits:2})}`;};
const rt=(iso)=>{if(!iso)return'';const d=new Date(iso),now=new Date(),dd=Math.floor((now-d)/86400000);if(dd===0)return d.toLocaleTimeString('en-US',{hour:'numeric',minute:'2-digit',hour12:true});if(dd===1)return'Yesterday';if(dd<7)return d.toLocaleDateString('en-US',{weekday:'short'});return d.toLocaleDateString('en-US',{month:'short',day:'numeric'});};
...
export default function ContactProfile(){
  ...
  const rEf=(x)=>setEf({name:x.name||'',phone:x.phone||'',phone_secondary:x.phone_secondary||'',email:x.email||'',company:x.company||'',role:x.role||'homeowner',preferred_contact_method:x.preferred_contact_method||'sms',billing_address:x.billing_address||'',billing_city:x.billing_city||'',billing_state:x.billing_state||'',billing_zip:x.billing_zip||'',insurance_carrier:x.insurance_carrier||'',policy_number:x.policy_number||'',claim_number:x.claim_number||'',referral_source:x.referral_source||'',notes:x.notes||'',tags:(x.tags||[]).join(', ')});
  ...
  const load=useCallback(async()=>{
    try{
      ...
      if(cj.length>0){const jI=cj.map(x=>x.job_id);const[jD,eD]=await Promise.all([db.select('jobs',`id=in.(${jI.join(',')})&order=created_at.desc&select=id,job_number,insured_name,phase,division,address,city,state,zip,date_of_loss,created_at`).catch(()=>[]),db.select('estimates',`job_id=in.(${jI.join(',')})&order=created_at.desc&select=id,job_id,estimate_number,estimate_type,status,amount,approved_amount,submitted_at,approved_at,denied_reason,pdf_url,notes`).catch(()=>[])]);const jm={};for(const x of cj)jm[x.job_id]=x;setJobs(jD.map(j=>({...j,_cRole:jm[j.id]?.role,_isPrimary:jm[j.id]?.is_primary})));setEsts(eD);} else{setJobs([]);setEsts([]);}
  ...
  const tabDefs=useMemo(()=>{const t=[{k:'conversations',l:'Conversations',n:convos.length},{k:'jobs',l:'Jobs',n:jobs.length},{k:'estimates',l:'Estimates',n:ests.length}];if(c?.role==='homeowner'||invs.length>0||pays.length>0)t.push({k:'financial',l:'Financial',n:invs.length+pays.length});t.push({k:'activity',l:'Activity',n:clog.length});return t;},[c,convos,jobs,ests,invs,pays,clog]);
  ...
  return(<PullToRefresh onRefresh={load}><div className="cp-page">
    ...
    <div className="cp-topbar">
      ...
      <button className="btn btn-secondary btn-sm" onClick={()=>setEditing(!editing)}><IconEdit style={{width:14,height:14}}/> {editing?'Cancel':'Edit'}</button>
    </div>
    ...
    <div className="cp-body">
      ...
      <div className="cp-sidebar">
        {editing ? (
          <EditForm form={ef} setForm={setEf} onSave={save} onCancel={()=>{setEditing(false);rEf(c);}} saving={saving} />
        ) : (<>
          {/* Identity */}
          <div className="cp-sidebar-section">
            <div className="cp-identity">
              ...
              <div>
                <div className="cp-name">{c.name||'Unknown'}</div>
          ...
          {/* Summary */}
          <div className="cp-sidebar-section">
            <div className="cp-section-label">Summary</div>
            <div className="cp-summary-grid">
              ...
              {c.referral_source&&<div className="cp-summary-stat"><div className="cp-summary-stat-label">Referral</div><div className="cp-summary-stat-value">{c.referral_source}</div></div>}
            </div>
          </div>
          ...
          <div className="cp-sidebar-section">
            ...
            <div className="cp-detail-rows">
              {c.phone&&<div className="cp-sidebar-row">
                <IconPhone className="cp-sidebar-row-icon"/>
                <div className="cp-sidebar-row-content"><div className="cp-sidebar-row-label">Phone</div><div className="cp-sidebar-row-value">{fp(c.phone)}</div></div>
                <div className="cp-sidebar-row-actions">
                  <a href={`tel:${c.phone}`} className="cp-sidebar-action" title="Call"><IconPhone style={{width:14,height:14}}/></a>
                  <a href={`sms:${c.phone}`} className="cp-sidebar-action" title="Text"><IconSms style={{width:14,height:14}}/></a>
                </div>
              </div>}
              {c.phone_secondary&&<div className="cp-sidebar-row">
                <IconPhone className="cp-sidebar-row-icon"/>
                <div className="cp-sidebar-row-content"><div className="cp-sidebar-row-label">Phone 2</div><div className="cp-sidebar-row-value">{fp(c.phone_secondary)}</div></div>
              </div>}
              {c.email&&<div className="cp-sidebar-row">
                <IconMail className="cp-sidebar-row-icon"/>
                <div className="cp-sidebar-row-content"><div className="cp-sidebar-row-label">Email</div><a href={`mailto:${c.email}`} className="cp-sidebar-row-value link" style={{color:'var(--accent)'}}>{c.email}</a></div>
              </div>}
            </div>
          </div>
          ...
          <div className="cp-sidebar-section">
            <div className="cp-section-label">Communication preferences</div>
            <div className="cp-detail-rows" style={{gap:'var(--space-3)'}}>
              <div className="cp-sidebar-row" style={{justifyContent:'space-between'}}>
                <div><div className="cp-sidebar-row-label">Text message consent</div><span className={`cp-opt-badge ${c.opt_in_status?'opted-in':'opted-out'}`}>{c.opt_in_status?'Opted in':'Not opted in'}</span></div>
              </div>
              <div className="cp-dnd-row">
                <div><div className="cp-dnd-label">Do Not Disturb</div><div className="cp-dnd-sub">Block all outbound messages</div></div>
                <button className={`conv-dnd-toggle${c.dnd?' on':''}`} onClick={tDnd}><div className="conv-dnd-knob"/></button>
              </div>
            </div>
          </div>

          {/* Addresses */}
          {addrCount>0&&<div className="cp-sidebar-section">
          ...
          {/* Insurance */}
          {(c.insurance_carrier||c.policy_number||c.claim_number)&&<div className="cp-sidebar-section">
            ...
            <div className="cp-detail-rows">
              ...
              {c.claim_number&&<div className="cp-sidebar-row"><IconFile className="cp-sidebar-row-icon"/><div className="cp-sidebar-row-content"><div className="cp-sidebar-row-label">Claim #</div><div className="cp-sidebar-row-value mono">{c.claim_number}</div></div></div>}
            </div>
          </div>}

          {/* Tags */}
          {tags.length>0&&<div className="cp-sidebar-section">
            <div className="cp-section-label">Tags</div>
            <div className="cp-tags-row">{tags.map((t,i)=><span key={i} className="cp-tag">{t}</span>)}</div>
          </div>}

          {/* Notes */}
          {c.notes&&!/^\[DEMO\]$/.test(c.notes)&&<div className="cp-sidebar-section">
            <div className="cp-section-label">Notes</div>
            <div className="cp-notes-preview">{c.notes}</div>
          </div>}
        </>)}
      </div>
...
/* ═══ EDIT FORM (replaces sidebar content) ═══ */
function EditForm({form,setForm,onSave,onCancel,saving}){
  ...
  const F=({label,field,type='text',placeholder})=>(<div className="form-group" style={{flex:1,marginBottom:0}}><label className="label">{label}</label>{type==='textarea'?<textarea className="input textarea" value={form[field]} onChange={e=>s(field,e.target.value)} rows={2} placeholder={placeholder}/>:<input ref={field==='name'?nr:undefined} className="input" type={type} value={form[field]} onChange={e=>s(field,e.target.value)} placeholder={placeholder}/>}</div>);
  ...
  return(<div className="cp-edit-form">
    ...
    <div className="cp-edit-row"><Sel label="Role" field="role" options={RO}/></div>
    <div className="cp-edit-row"><Sel label="Preferred Contact" field="preferred_contact_method" options={CM}/></div>
    ...
    <div className="cp-edit-row"><F label="City" field="billing_city"/></div>
    <div className="cp-edit-row"><F label="State" field="billing_state"/><F label="ZIP" field="billing_zip"/></div>
    <div className="cp-edit-section-label">Insurance</div>
    <div className="cp-edit-row"><F label="Carrier" field="insurance_carrier" placeholder="State Farm..."/></div>
    <div className="cp-edit-row"><F label="Policy #" field="policy_number"/></div>
    <div className="cp-edit-row"><F label="Claim #" field="claim_number"/></div>
    <div className="cp-edit-section-label">Other</div>
    <div className="cp-edit-row"><F label="Referral" field="referral_source" placeholder="Google, agent..."/></div>
...
/* ═══ TAB: ESTIMATES (grouped by job) ═══ */
function EstTab({ests,jobs}){
  const[exp,setExp]=useState({});
  const grp=useMemo(()=>{const g={};for(const e of ests){if(!g[e.job_id])g[e.job_id]=[];g[e.job_id].push(e);}return g;},[ests]);
  ...
  if(!ests.length)return<div className="empty-state"><div className="empty-state-icon">📝</div><div className="empty-state-title">No estimates</div><div className="empty-state-text">Estimates linked to this contact's jobs will appear here.</div></div>;
  ...
  return(<div className="cp-estimates">
    ...
    {Object.entries(grp).map(([jid,es])=>{const j=jm[jid];const o=exp[jid];return(<div key={jid} className="cp-est-group">
      <button className="cp-est-group-header" onClick={()=>setExp(p=>({...p,[jid]:!p[jid]}))}><div className="cp-est-group-left"><span className="cp-est-group-emoji">{DE[j?.division]||'\u{1F4C1}'}</span><div><div className="cp-est-group-title">{j?.job_number||'Unknown'}</div><div className="cp-est-group-sub">{j?.insured_name||''} \u2014 {es.length} est.</div></div></div><IconChevronDown style={{width:18,height:18,transition:'transform 200ms ease',transform:o?'rotate(180deg)':'rotate(0)'}}/></button>
