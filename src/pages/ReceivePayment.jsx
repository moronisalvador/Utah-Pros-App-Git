...
export default function ReceivePayment() {
  ...
  if (loadError) return <div className="coll-page"><ErrorState message={loadError} onRetry={load} secondary={<GhostButton onClick={() => navigate('/collections?tab=payments')}>Back to payments</GhostButton>} /></div>;
  if (receipt) return <div className="coll-page" style={{ overflowX: 'clip' }}><header className="coll-header"><div><h1 className="coll-title">Payment received</h1><div className="coll-subtitle">The receipt is recorded in UPR and QuickBooks.</div></div></header><CollCard><h2 style={{ marginTop: 0 }}>Receipt complete</h2><p>UPR receipt <b>{receipt.receipt_id || '—'}</b> · QuickBooks payment <b>{receipt.qbo_payment_id || '—'}</b></p><div><GhostButton onClick={() => navigate('/collections?tab=payments')}>Return to Payments</GhostButton></div></CollCard></div>;
  if (contactId && !(data?.invoices || []).length) return <div className="coll-page"><EmptyState icon="💵" title="No open invoices" sub="This customer has no open QuickBooks-linked invoices to receive a payment against." action={<GhostButton onClick={() => navigate('/collections?tab=payments')}>Back to Payments</GhostButton>} /></div>;
  return <div className="coll-page" style={{ overflowX: 'clip' }}><header className="coll-header"><div><h1 className="coll-title">Receive payment</h1><div className="coll-subtitle">Create one QuickBooks payment and allocate it across open invoices.</div></div><div className="coll-actions"><GhostButton onClick={() => navigate('/collections?tab=payments')}>← Payments</GhostButton></div></header>{data && <ReceivePaymentForm key={`${contactId || 'contacts'}:${invoiceId || 'none'}`} data={data} prefillInvoice={invoiceId} onSubmit={submit} onSelectContact={(id) => navigate(`/collections/receive-payment?contact=${encodeURIComponent(id)}${invoiceId ? `&invoice=${encodeURIComponent(invoiceId)}` : ''}`)} submitting={submitting} />}</div>;
}
