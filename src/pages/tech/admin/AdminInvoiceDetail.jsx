...
export default function AdminInvoiceDetail() {
  ...
  const {
    db,
    isFeatureEnabled,
    isStrictFeatureEnabled,
    user,
  } = useAuth();
  const billingEnabled = isFeatureEnabled('feature:billing');
  const lineCommandsEnabled = isStrictFeatureEnabled(
  ...
  const sendRouteRef = useRef({ invoiceId, epoch: 0, token: 0, activeToken: null, alive: true });
  ...
  useEffect(() => {
    sendRouteRef.current.alive = true;
    ...
    return () => {
      loadRequestRef.current += 1;
      sendRouteRef.current = { ...sendRouteRef.current, alive: false, epoch: sendRouteRef.current.epoch + 1, activeToken: null };
      sendInFlightRef.current = false;
    };
  }, [load]);
  ...
  const sendInvoice = async () => {
    ...
    const isCurrentRoute = () => {
      ...
      return current.alive && current.invoiceId === invoiceId && current.epoch === routeEpoch && current.activeToken === token;
    };
  ...
  return (
    ...
    >
      ...
      {!synced && canAct && balance > 0.005 && (
        <div className="am-section-note">
          {lineCommandsEnabled
            ? 'Draft — edit a line item and save to QuickBooks before sending or recording payment.'
            : 'Draft line editing is temporarily unavailable during a QuickBooks update.'}
        </div>
      )}
      ...
      <DocumentLineList
        lines={lines}
        editable={!inv.locked && lineCommandsEnabled}
        lineHref={(line) => adminInvoiceLineHref(invoiceId, line.id)}
        onLineClick={() => impact('light')}
        onAdd={() => { impact('light'); navigate(adminInvoiceLineHref(invoiceId, 'new')); }}
