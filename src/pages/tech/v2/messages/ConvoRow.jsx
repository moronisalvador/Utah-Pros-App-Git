 ...
 *   a tech can read state from three feet away. Tapping the row opens the thread.
 *
 * WHERE IT LIVES:
 *   Route:        n/a (list item)
 ...
 * DEPENDS ON:
 *   Packages:  react
 *   Internal:  ./msgsSelectors (convoUnread), ./msgDateUtils (listTime)
 *   Data:      reads/writes → none (pure presentation)
 * ════════════════════════════════════════════════
 */
import React from 'react';
import { convoUnread } from './msgsSelectors';
...
export default function ConvoRow({ conv, onOpen }) {
  ...
  return (
    ...
    >
      ...
      <span className="tv2-msgs-row__avatar">{initials(conv.title)}</span>
      <span className="tv2-msgs-row__main">
        ...
        <span className="tv2-msgs-row__preview">{preview || ' '}</span>
      </span>
