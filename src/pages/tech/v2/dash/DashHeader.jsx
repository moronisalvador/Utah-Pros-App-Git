  ...
  return (
    <div className="tv2-dash-header">
      ...
      <div className="tv2-dash-header__menu" onBlur={(e) => { if (!e.currentTarget.contains(e.relatedTarget)) setShowMenu(false); }}>
        <button type="button" className="tv2-dash-header__icon-btn" data-active={showMenu ? 'true' : undefined} aria-label="More" onClick={() => setShowMenu((v) => !v)}>
        ...
        {showMenu && (
          <div className="tv2-dash-menu">
            ...
            <button type="button" className="tv2-dash-menu__item" data-danger={confirmLogout ? 'true' : undefined} onMouseDown={(e) => e.preventDefault()} onClick={() => { setShowMenu(false); handleLogoutTap(); }}>
      ...
      <div className="tv2-dash-header__name">Hey {firstName} 👋</div>
      <div className="tv2-dash-header__summary">{count} appointment{count !== 1 ? 's' : ''} today</div>
    </div>
  );
}
