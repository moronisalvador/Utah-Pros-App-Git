  ...
  return (
    <div style={S.page}>
      ...
      {jobPickerModal && (() => {
        ...
        const matchesJob = (j) => !q || (j.insured_name || '').toLowerCase().includes(q) || (j.job_number || '').toLowerCase().includes(q) || (j.address || '').toLowerCase().includes(q);
        const fmtLoss = (d) => d ? new Date(d + 'T12:00:00').toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) : '';
        const pj = boardData.filter(matchesJob);
        const oj = panelJobs.filter(j => !boardData.some(b => b.job_id === j.id) && matchesJob(j));
        return (
