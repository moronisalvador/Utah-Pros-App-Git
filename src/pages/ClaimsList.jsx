...
const fmt$ = (v, showZero = false) => { const n = Number(v); if (isNaN(n) || (!n && !showZero)) return '—'; if (n === 0) return '$0'; if (n >= 1000) return '
    ...
    <div className="page-container">
      ...
      <div style={{ display: 'flex', gap: 1, background: 'var(--border-color)', borderBottom: '1px solid var(--border-color)', overflowX: 'auto' }}>
        {[
          ...
          { label: 'Outstanding',   value: fmt$(stats.outstanding, true),         color: stats.outstanding > 0 ? '#dc2626' : '#059669', isNum: true },
        ].map(k => (
        ...
        {filtered.length === 0 ? (
          ...
          <>
            ...
            <div style={{ padding: '10px 16px', fontSize: 11, color: 'var(--text-tertiary)', borderTop: '1px solid var(--border-color)', background: 'var(--bg-secondary)' }}>
              {filtered.length} claims · Outstanding: <strong style={{ color: filtered.reduce((s,c) => s + Number(c.total_balance||0), 0) > 0 ? '#dc2626' : '#059669' }}>{fmt$(filtered.reduce((s,c) => s + Number(c.total_balance||0), 0), true)}</strong>
            </div>
          </>
        )}
      </div>
