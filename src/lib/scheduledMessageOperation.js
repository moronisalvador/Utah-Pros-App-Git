 ...
 *   - A changed conversation, body, or send time creates a different operation.
 * ════════════════════════════════════════════════
 */
...
export function getScheduledMessageOperation(
  ...
  const existingId = operations.get(key);
  if (existingId) return { key, id: existingId, storageKey };

  if (storageKey && isPwaOwnerLeaseCurrent(ownerLease, resolvedStorage)) {
    try {
      const durableId = resolvedStorage?.getItem(storageKey);
      if (durableId) {
        operations.set(key, durableId);
        return { key, id: durableId, storageKey };
      }
    } catch {
...
export function clearScheduledMessageOperation(
  ...
  if (operation?.key) operations.delete(operation.key);
