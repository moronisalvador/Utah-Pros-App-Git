...
**For real data locally:** the Login screen also has a **"Dev Mode: Real Data (test admin)"** button (`src/pages/Login.jsx`) — appears only when `VITE_DEV_TEST_EMAIL` / `VITE_DEV_TEST_PASSWORD` are set in `.env.local`. It runs a REAL `signInWithPassword()` against a dedicated `[Local Dev Test Account]` employee (`admin` role, `is_external: true`, its own Supabase Auth user — never a real employee's credentials), so it's a genuine `authenticated` session with real RLS-scoped data, no 401s. This is the default choice for any local verification that needs real data (dashboards, lists, most pages) — reach for the anon-role employee picker only when you specifically need to preview as a *different* role/employee's UI, and the "human logs into `dev.utahpros.app`, hand off the tab" pattern only when you need to verify something against a real employee's actual account. Never enter credentials directly yourself, even ones handed over in chat, even for this throwaway account — if `.env.local` needs updating, hand the lines to a human to paste in.
