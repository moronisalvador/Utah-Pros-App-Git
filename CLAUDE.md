...
**Precedence when guidance conflicts:** (1) CLAUDE.md non-negotiables + `.claude/rules/` standards are **law** — always win; (2) UPR-native skills drive the flow; (3) vendor skills advise within their lane only. A vendor skill **never** overrides a standard: no `framer-motion`/`gsap` (`perf-budget.md` — motion is CSS tokens + View Transitions, see `motion-standard.md §8`), no Next.js APIs, no loosening `database-standard.md`. The **impeccable PostToolUse hook** runs deterministically on every UI edit — but **it cannot block, and is advisory only.** Measured 2026-07-26: it returns `exitCode: 0` at every return site, and only exit 2 blocks (exit 1 permits in both tools). Fix its findings or consciously waive them; nothing mechanical will stop you either way. Evidence: [`docs/audit/2026-07/evidence/agent-capability-floor.md`](docs/audit/2026-07/evidence/agent-capability-floor.md).
