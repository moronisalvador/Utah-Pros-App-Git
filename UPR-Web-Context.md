...
functions/
  api/                            — 58 files total; only the SMS/Esign/Encircle/demo-sheet workers below are
    ...
    send-message.js               — Outbound SMS with TCPA compliance + DND guard. **Wave -1 hotfix (Jul 9 2026):** the `skip_compliance` param + gate were REMOVED (F-2) — the DND + opt-in chain now runs for every outbound message, no bypass. Group/broadcast conversations are hard-refused (400 `MULTI_RECIPIENT_UNSUPPORTED`) before the legacy participants[0]-only check (F-4); the real per-participant consent loop is Phase B.
