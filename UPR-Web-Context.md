...
functions/
  api/                            — 58 files total; only the SMS/Esign/Encircle/demo-sheet workers below are
    ...
    attest-sms-consent.js         — Record-only admin/office endpoint for verified prior SMS permission. Actor is session-derived; method/date/evidence are required; the service-only atomic RPC refuses DND/STOP/opt-out and writes `contacts` + `sms_consent_log`. It has no provider/send capability. Migration `20260724014423_attest_prior_sms_consent.sql` is authored but not applied.
