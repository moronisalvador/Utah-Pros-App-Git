...
src/
  ...
  components/
    ...
    **App-local Swift Capacitor plugins** (`ios/App/App/*.swift`, all registered in `AppViewController.capacitorDidLoad()`, all hand-wired into the classic `project.pbxproj` at four insertion points — PBXBuildFile, PBXFileReference, group children, Sources phase — and all feature-detected from JS via `Capacitor.isPluginAvailable` so an older installed binary degrades silently). Wiring is pinned end to end by `tests/qa/unit/native-plugin-wiring.test.js`. **Every overlay presents `.overFullScreen`, NEVER `.fullScreen`** — fullScreen unmounts the WKWebView, firing `visibilitychange:hidden` into the SPA and tearing down live state underneath (sim-reproduced).
    ...
    · `NativeCameraExperience` (`src/lib/nativeCamera.js`, 2026-08-14) — the WhatsApp-style camera every photo button opens: full-screen AVCaptureSession viewfinder (shutter / 3-state flash / front-back flip), a PHCachingImageManager recent-photos strip (multi-select with numbered badges + "Add N photos" confirm in album mode; tap-to-return in single mode), and an album icon → `PHPickerViewController` multi-select (no photo permission needed; picker-cancel returns to the camera, not the page). `capture({allowMultiple})` resolves `{photos:[{webPath,format}]}` — every photo re-rendered orientation-up and JPEG-encoded in Swift so the web pipeline never sees HEIC or EXIF rotation; ✕ rejects with "cancel" (silent no-op via `isUserCancelled`). Camera permission denied / simulator: inline "enable access"/"unavailable" state with Open Settings, strip and album still usable. Library permission denied: strip hides, album icon still works (PHPicker). Shutter is always snap-first — returns exactly one photo immediately, both modes.
...
- **Plugins installed:**
  ...
  - **Multi-photo album selection (2026-08-13, entry UX reworked 2026-08-14):** the five album-oriented surfaces (TechJobAlbum, TechClaimAlbum, TechRoomDetail, TechJobDetail, TechClaimDetail) select several photos in one pass — natively via the camera experience's strip/album (`allowMultiple: true`) or the adjacent album icon → `pickNativePhotos()` (= `Camera.chooseFromGallery({allowMultipleSelection:true})` with a `pickImages` fallback for older binaries); web via the `multiple` album input. (`src/components/tech/AddPhotoSourceSheet.jsx` — the custom Take photo / Choose from album sheet — was deleted with the rework; on multi-job claim surfaces the job-picker sheet remains, because "which job?" is attribution, not a source chooser, and it now carries the tapped flow (camera vs album) through the pick.) Files upload **sequentially** through each page's `uploadOne` (per-file 10 MB/image-type/offline guards throw so the loop counts them), which delegates to the shared `usePhotoUpload` hook — so photos **compress before storage** (perf-budget §2) via `mediaCompress.js` — with a summary toast — "N photos uploaded" / "K of N photos uploaded — M failed" — and a running "Uploading K of N…" on the button. **Since the 2026-08-14 close-out gauntlet, ALL EIGHT photo surfaces route through the hook** — the three quick-capture surfaces' hand-rolled raw Storage POSTs were consolidated too (they shipped every snap uncompressed), and `src/lib/offlineQueueSourceContract.test.js` now pins pages/tech/ at ZERO raw uploaders. The same gauntlet gave the three inline multi-job job-picker sheets (TechClaimAlbum/TechClaimDetail/TechRoomDetail) the `useSheetClosing` enter/exit contract (TechClaimDetail latches its last picker payload for the closing frames), pinned in `sheet-exit-animation.test.js`. The quick-capture surfaces (PhotoCaptureButton, HubDock, TechAppointment) deliberately stay single-shot (`openNativeCameraExperience()` without allowMultiple — the shutter returns exactly one photo, uploaded immediately; strip taps return that one photo; the built-in picker is capped at one); `tests/qa/unit/album-multi-photo-select.test.js` pins the camera-first doctrine and the split both directions, and `native-plugin-wiring.test.js` pins the plugin's four pbxproj insertion points, registration, JS name, and `.overFullScreen`. i18n: `tech:toast.photosUploaded/photosPartial/photosFailed/albumError`, `tech:btn.uploadingCount`, `tech:photoSource.choose` (the album icon's label; the other photoSource keys went with the sheet) in en/pt/es.
  - **Messages composer attach flow (2026-08-14):** the tech Messages composer (`src/pages/tech/v2/messages/Composer.jsx`, rendered by ThreadView) joined the camera-first doctrine as a third tier — *attach flows*. On a plugin-carrying binary its Attach photos action opens `openNativeCameraExperience({ allowMultiple: true })`; the returned Files feed `useComposerAttachments.addFiles` exactly like input-picked ones (staged preview thumbnails, removable before send — attachments stay staged, upload timing and `POST /api/send-message` untouched). It gates on `nativeCameraExperienceAvailable()` — **the no-plugin fallback is the plain `multiple` file input, never `captureNativePhoto()`** (the single shot has no album; an attach flow losing album access is a regression). Web/PWA keeps the plain input with no `capture` attribute. Camera errors toast via `tech:toast.cameraError`; cancel is a silent no-op (`isUserCancelled`). Pinned as the `ATTACH_FLOW_SURFACES` tier in `tests/qa/unit/album-multi-photo-select.test.js`. **Same day, owner-directed:** the composer's [+] itself became Apple's own action sheet on plugin-carrying binaries — Take Photo / Photo Library / Templates / Internal note — via the `NativeActionMenu` plugin (see the app-local plugin list); the web sheet stays as the web implementation and the every-fallback path.
