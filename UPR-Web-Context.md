...
- **Plugins installed:**
  ...
  - `@capacitor/push-notifications` — `src/lib/pushNotifications.js` registers + upserts to `device_tokens`; APNs delivery runs through `functions/api/send-push.js`. Settings → Notifications has separate channel-specific controls: browser/PWA Turn on/Turn off manages only Web Push, while the Capacitor build manages only this installation's APNs binding. Native intent is bound to the verified opaque owner lease, so a different account on the same phone defaults off; a legacy boolean migrates on only with an existing same-owner local binding. Explicit native off survives restart, blocks login bootstrap before permission/APNs/database work, persists before detach, unregisters locally, and asks iOS to remove delivered banners. The owner-bound v2 cleanup journal distinguishes provisional enrollment from confirmed binding: same-owner registration is single-flight, a confirmed overlap receives a final post-settlement delete, only a parsed top-level PostgREST `42501` plus the full canonical foreign-owner message releases a provisional marker, and malformed/nested/partial-code/other/network-ambiguous failures remain journaled for later same-owner reconciliation. Settings silently rechecks iOS permission on resume and never labels denied, unknown, or cleanup-pending delivery as On; the status surface never returns the token. APNs serializes only generic `Utah Pros notification` / `Open Utah Pros for details.` alert copy, one server-allowlisted route, and an opaque recipient binding; unsafe/encoded/oversized/credential/query-bearing routes and the public signing bearer paths `/sign/:token` and `/s/:code` fall back to `/` before Apple, and a tap is dropped unless it matches the current employee. Those signing paths remain valid Universal/App Links but are never Push payload data. Appointment Push audiences are structural: assigned intersects the named employee with current crew, updated/canceled resolve current crew, and `recipient_ids` cannot widen them. `capacitor.config.json` requests native foreground badge, sound, and generic alert presentation while the JavaScript listener emits only a content-private refresh signal; browser/PWA delivery is unaffected. Source supports exact sandbox/production separation; the two focused native Push migrations are applied/live-verified, while final clean-source archive, TestFlight production-token delivery/tap proof, and App Store release remain separate gates. Broad S1h is not an activation prerequisite.
...
**2026-07-28 native device-control update:** the reviewed but uncommitted
Settings → Notifications Turn on/Turn off batch was rebuilt for the native
sandbox path, synchronized into Capacitor with zero tracked native-project
drift, signed as Debug, and installed in place over Wi-Fi on the owner's iPhone
17 Pro Max. Device tooling confirmed bundle `com.utahprosrestoration.upr`,
version `1.0.0 (1)`, launched and running. This closes only the
build/sync/sign/install/launch step. Physical confirmation of Turn on, denied
permission after resume, Turn off, restart persistence, background delivery,
and tap routing remains open; the source batch is still uncommitted, unpushed,
and undeployed.
