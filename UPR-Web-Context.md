...
**Status:** foundation + push worker + Billing UI + AR mapping trigger live on prod, validated (real QBO invoice created/deleted; AR-sync trigger verified: draft ignored, push sets `invoiced_value`/date, legacy values preserved). **Remaining 2a:** flip `auto_draft_invoices` → `'true'` once Moroni has tested the Billing UI on prod. **2b:** UPR invoice editing UI (line items, adjustments) + two-way sync. **2c:** payments sync (per-category collected/outstanding → `jobs.collected_value`). **Future:** optionally point the dashboard directly at the `invoices` table (it already carries deductible/depreciation/insurance-vs-homeowner columns) and retire the hand-entered Revenue fields.
