...
src/
  ...
  pages/
    ...
    components/collections/       — Collections redesign pieces: collTokens.js (page-scoped UPR palette + $/date
                                    ...
                                    invoice & estimate builders). Styles
...
- **Single full-width column** (no lateral panels): a header `CollCard` carries the eyebrow
  ...
  date** — UPR `invoices.due_date`, does NOT sync back from QBO). Estimates also show Type + service address.
...
- **Invoice payment summary** (full-width `CollCard` below the builder): Invoiced / Collected / Balance KPIs
  + `ProgressBar` + payment history (`payments?invoice_id=eq.…`) + inline **two-click Record payment**
  (insert `payments` → `POST /api/qbo-payment {payment_id}`; delete via `{payment_id, action:'delete'}`).
  Estimates have no payments; instead a "→ Convert to invoice" action.
...
- **Deferred (Phase 2, needs schema + QBO worker):** editable customer memo / terms / PO. Until then the
  customer memo + service address are auto-generated when the doc is pushed to QBO (shown read-only).
