...
functions/
  api/                            — 58 files total; only the SMS/Esign/Encircle/demo-sheet workers below are
    ...
    attest-sms-consent.js         — GET exposes only the authoritative consent decision to employees with conversations access; POST is record-only for internal admin/office. Actor and trusted Cloudflare IP are server-derived; method/date/evidence are required. Service-only RPCs serialize with inbound STOP projection, check duplicate-contact suppression, write `service_sms_consents` + append-only `sms_consent_log`, and never change global opt-in or call a provider. Migration `20260724014423_attest_prior_sms_consent.sql` is authored but not applied.
...
service_sms_consents    — Pending service-role-only table (migration authored, not applied) for
                          exact-phone staff P2P permission. Forced RLS, no browser policy, fixed
                          service scope/version/sender identity; attestation appends
                          `sms_consent_log` and refuses duplicate-phone DND/opt-out/pending STOP.
