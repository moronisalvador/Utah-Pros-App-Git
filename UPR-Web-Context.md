...
functions/
  api/                            — 58 files total; only the SMS/Esign/Encircle/demo-sheet workers below are
    ...
    sync-houzz.js                 — Push a reconstruction-division job to Houzz Pro. POST { job_id, force? }. Houzz Pro has no public API, so this POSTs the job (customer name/email/phone/address + job_number/claim_number/insurance_company/type_of_loss) to a Zapier webhook (env HOUZZ_ZAPIER_WEBHOOK_URL, Catch Hook trigger → Houzz Pro "Create Project" action, set up in Zapier's UI — not buildable via API). Idempotent (skips if jobs.houzz_synced_at already set, unless force:true). Writes jobs.houzz_sync_status/houzz_synced_at/houzz_sync_error. No houzz_project_id — Zapier's Zap runs asynchronously so there's no way to read a created project's ID back; "sent" means "handed off successfully," not "confirmed created." Called automatically from CreateJobModal + TechNewJob post-RPC when division==='reconstruction'; no backfill/retry UI yet.
