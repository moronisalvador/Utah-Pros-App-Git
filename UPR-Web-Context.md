...
- **Invoice payment summary** (full-width `CollCard` below the builder): Invoiced / Collected / Balance KPIs
  ...
  saving updates the `payments` row and re-syncs QBO by **delete + recreate** (the `/api/qbo-payment` worker
  has create + delete only, no update); Delete lives inside that edit form (two-click). The form opens from
  the **Receive payment** toolbar button (no more full-width "+ Record payment"; no per-row Delete button).
