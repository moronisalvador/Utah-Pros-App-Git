...
src/
  ...
  pages/tech/
    ...
    TechClaimAlbum.jsx            — Field tech claim photo album at /tech/claims/:claimId/photos. Slim sticky top bar (back + "Photos" + claim#/insured subtitle + count badge), division-tinted accent strip, 2-column thumbnail grid (~160×160) with per-job grouping on multi-job claims, relative/short-date timestamp caption below each thumbnail ("2h ago" → "Mar 28"), pinned bottom Add Photo button with multi-job sheet picker. Local copy of Lightbox (2 callers — promote to components/tech/ when TechJobDetail adds the 3rd).
...
10. **TechJobDetail follow-up (TECH-JOB-DETAIL-TASK)** — `/tech/jobs/:jobId` currently renders desktop `JobPage`, same "web-packed-into-mobile" problem that was fixed for claims. Planned follow-up: build `TechJobDetail.jsx` reusing the local components (Hero, ActionBar, NowNextTile, PhotosGroup, Lightbox, DetailRow) introduced in TechClaimDetail. The Photos & Notes section should also include a "See all →" entry point to a job-scoped album (mirror `/tech/claims/:id/photos`, likely `/tech/jobs/:id/photos`). At that point, Lightbox will have 3 callers — promote to `src/components/tech/Lightbox.jsx` then.
