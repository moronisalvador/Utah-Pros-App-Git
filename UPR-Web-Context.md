...
- `sync-claim-to-encircle` (Apr 18 2026) — pushes UPR-native claims UP to Encircle. Fired automatically from CreateJobModal + TechNewJob after `create_job_with_contact` RPC succeeds. Idempotent via `claims.encircle_claim_id`. Failures stored on `claims.encircle_sync_error` and surfaced in DevTools → Backfill → Unsynced Claims panel with per-row retry. On success writes Encircle id back to `claims.encircle_claim_id` AND all child `jobs.encircle_claim_id`.
  ...
  - **Future hardening (not yet built):** a server-side safety-net sweep (re-pushing any claim with `encircle_claim_id IS NULL AND encircle_sync_error IS NULL` and no Encircle-linked sibling job) would remove the last browser dependency. Blocked this session: no `wrangler.toml`/cron in-repo (heartbeat config lives in the Cloudflare dashboard), `ENCIRCLE_API_KEY` only exists in the Pages worker (not the DB), `pg_cron` not installed, and a DB→worker call would need a new Cloudflare auth env var. Activating it needs Cloudflare dashboard access (point a cron at a `{sweep:true}` endpoint, or confirm the existing `/api/process-scheduled` heartbeat to piggyback on).
