...
src/
  ...
  pages/
    ...
    Conversations.jsx             — SMS/MMS messaging (GHL-style, TCPA compliant). **Wave -1 hotfix (Jul 9 2026):** `handleSend` now checks `res.ok` BEFORE parsing the body and the worker-failure fallback that inserted a `status:'queued'` ghost `messages` row was DELETED (F-1) — the worker is the sole writer of `sms_*` rows. On any send failure it surfaces the real error via `window.dispatchEvent(new CustomEvent('upr:toast', {detail:{message,type:'error'}}))` and appends no optimistic bubble. **Prior-consent remediation (authored Jul 23 2026; migration not applied):** admin/office users see a consent banner and `SmsConsentAttestationModal` for verified verbal permission, signed work authorization or other evidenced permission. It records method/date/note and can retry the same blocked optimistic send only after the server records consent; STOP/DND cannot be cleared.
...
functions/
  api/                            — 58 files total; only the SMS/Esign/Encircle/demo-sheet workers below are
    ...
    send-message.js               — Outbound SMS chokepoint with TCPA compliance + DND guard. **Wave -1 hotfix (Jul 9 2026):** `skip_compliance` param + gate REMOVED (F-2) — the DND + opt-in chain runs for every outbound message, no bypass. **SMS-experience Phase B (Jul 9 2026):** the Wave -1 group/broadcast refuse-guard is replaced by the real **per-participant consent loop** — every participant is DND+opt-in gated *before* being texted (a DND/opted-out participant beyond index 0 is never sent to), and each recipient gets its OWN `messages` row so a per-recipient send failure is recorded instead of vanishing. Worker is the sole writer of `sms_*` rows; a recipient with no valid phone is refused, never cross-channel-retargeted. **Messaging transport foundation (Jul 23 2026):** shared Worker authorization and the conversations capability run before service-role/provider access; actor identity is server-derived; stable client request IDs, provider adapters, and the live attempt/event/outbox foundation preserve consent, sole-writer, and no-fallback rules. **Prior-consent remediation (authored Jul 23 2026):** explicit `opt_out_at` has its own non-remediable failure code; staff P2P may consume exact-phone purpose-scoped evidence without enabling scheduled, automated, group or campaign sends. Preview is CallRail-only for owner-controlled verification while Production is disabled. Plan: `docs/messaging-transport-roadmap.md`.
    attest-sms-consent.js         — Record-only admin/office endpoint for verified prior staff person-to-person SMS permission. Actor and trusted request IP are server-derived; method/date/evidence are required; the service-only atomic RPC locks authority/contact rows, refuses DND/STOP/opt-out, updates only scoped contact fields, and retains deduplicated evidence in `sms_consent_log`. It has no provider/send capability. Migration `20260724014423_attest_prior_sms_consent.sql` is authored but not applied.
...
sms_consent_log         — TCPA opt-in/out audit log
                          Pending `attest_prior_sms_consent` RPC (migration authored, not applied)
                          atomically records verified verbal/signed-work-authorization/other prior
                          permission with consent date, evidence, employee actor and server timestamp;
                          service-role-only and refuses DND or explicit opt-out.
