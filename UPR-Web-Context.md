...
src/
  ...
  lib/
    ...
    useSheetClosing.js            — Aug 14 2026: exit-animation half of the tech bottom-sheet contract (motion-standard §3 "every enter has an exit"; the focus/Escape half is useDialogLifecycle.js). useSheetClosing(open) keeps the sheet mounted through its ~165ms exit (`present = open || closing`), swaps the .tech-sheet-overlay/.tech-sheet-panel classes to their --closing variants (tech-fade-out / tech-slide-down keyframes in index.css, calc(base*0.75) on --motion-ease-accelerate, forwards), and unmounts on a name-filtered animationend with a 240ms safety timer (reduced motion: CSS `animation: none` + a 0ms task — same contract as Modal.jsx, whose closing mechanism this lifts). Adopters: AddRoomSheet, AddPhotoSourceSheet, ClockSupersedeSheet (which latches its last `precheck` as render-adjusted state so the closing frames keep their text). The other four sheets (ReadingEntry/PhotoNote/EquipmentPlacement/Esign) still unmount instantly — flagged follow-up. Contract: tests/qa/unit/sheet-exit-animation.test.js.
