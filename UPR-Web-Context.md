...
functions/
  ...
  lib/
    ...
    process-callrail-events.js    — Scheduler-secret HTTP recovery worker for retained CallRail text events. Reclaims only due/stale work with bounded backoff and retries atomic canonical projection without making a provider send. Owner-applied migration `20260724002500_callrail_event_recovery_scheduler.sql` supplies the five-minute exact-URL pg_cron/pg_net invoker. Live PATCH response drift then required `20260724051500_claim_callrail_provider_event.sql`: the service-role-only invoker RPC atomically claims and returns one event so a mutation cannot be mistaken for a lost claim. A separate read-only reconciliation worker polls exact CallRail history matches for accepted/ambiguous sends.
