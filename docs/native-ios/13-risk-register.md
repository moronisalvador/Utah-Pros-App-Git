...
| NIOS-029 | Capacitor maintenance stops before native replacement is proven | High | Open | close `NIOS-H`; retain named operational owner, critical-fix policy, independent release path and material-drift reopen rule until cutover | Gate H and whole initiative / product owner |
...
| NIOS-034 | Swift implementation starts while the operational PWA/Capacitor fallback still has blocking security, data-preservation, release, or device gaps | High | Open | require independently reviewed `NIOS-H: READY` before any committed Swift project or implementation branch | Gate H / product owner + current-client hardening owner |
| NIOS-035 | “Harden first” becomes an undefined perfection gate that delays native work for unrelated polish or refactors | Medium | Open | use explicit P0/P1 closure, supported-scope exclusions, aggregate-risk review, and owned P2/Medium/Low deferrals | Gate H / product owner + orchestrator |
