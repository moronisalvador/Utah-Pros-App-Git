 ...
 * DEPENDS ON:
 *   Packages:  none
 *   Internal:  docs/mobile-production-readiness-roadmap.md, CLAUDE.md, AGENTS.md
...
The project provides four reusable roles:

| Role | Default mode | Purpose |
|---|---|---|
| `mobile-readiness-mapper` | read-only | Trace routes, callers, contracts, migrations, workers, policies, and documentation before edits. |
| `mobile-readiness-security-reviewer` | read-only | Challenge authentication, authorization, Storage, privacy, compliance, and secret boundaries. |
| `mobile-readiness-contract-tester` | read-only source | Design/run negative, failure, compatibility, and regression checks without changing source. |
| `mobile-readiness-release-auditor` | read-only | Verify evidence, release gates, device claims, rollback, and honest limitations. |

The primary is the only default writer. If a session needs more than one writer, it must divide work
into disjoint file sets and record the ownership table before editing.
