...
| 10 | **Agent QA access P2+** | P1 shipped; hosted `qa-staging` CI is live with three rotated signed-in fixtures and a zero-failure gate (latest: 163 / 375 passed, 0 failed, 212 skipped); P2a's local runtime never landed | Little — local replay, six local-only SQL proofs, and the hosted skip/conversion tail remain | **FOLD IN.** Retire the phase plan; the staging runbook, target-policy extension, and remaining test conversions are the living successors. |
...
2. Finish the current database hardening tail on `qa-staging` (#4), converting the remaining
   hosted skips while preserving the zero-failure CI gate.
