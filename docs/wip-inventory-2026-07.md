...
| 10 | **Agent QA access P2+** | P1 shipped; hosted `qa-staging` CI is live with three rotated signed-in fixtures. Raw latest: 163 / 375 assertions passed, 0 failed, 212 skipped, plus 46 setup errors across 44 files / 90 suite nodes; P2a's local runtime never landed | Little — local replay, six local-only SQL proofs, and the hosted setup/skip conversion tail remain | **FOLD IN.** Retire the phase plan; the staging runbook, target-policy extension, and remaining test conversions are the living successors. |
...
2. Finish the current database hardening tail on `qa-staging` (#4), converting the failed setup
   suites and hosted skips while preserving zero failed assertions and ratcheting suite debt down.
