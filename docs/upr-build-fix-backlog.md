...
| # | Work | Registry ID | Why | Size |
|---|---|---|---|---|
| 6.1 | **Isolated QA execution — hosted live; local replay open** | `QA-002` / `UPRF-QA-001` | The persistent `qa-staging` branch is seeded, protected by exact-target refusal, and wired to CI with rotated standing identities. At commit `19b3e64a`, hosted CI discovered the 78 JavaScript suites and reported 163 / 375 assertions passed, 0 failed, and 212 skipped. Zero failed assertions is now the hard gate. Remaining: convert the skip tail, complete the role/reset matrix, and build the governed local bootstrap needed to execute the six SQL/pgTAP proofs. The shared project remains forbidden for test writes. | M |
| 6.1a | ✅ **Replace the dark-lane alarm with truthful coverage contracts** | — | Shipped: `tests/qa/unit/db-lane-coverage.test.js` proves hosted JavaScript discovery, the zero-known-failure runner, and the exact six-file local-only SQL inventory. Runtime pass/skip truth comes from the hosted receipt, not static source. | ✅ done |
