...
| # | Work | Registry ID | Why | Size |
|---|---|---|---|---|
| 6.1 | **Isolated QA target — use a Supabase BRANCH, not local Docker** | `QA-002` / `UPRF-QA-001` | **[recon/2026-07-26] 75 files under `supabase/tests/` never execute in CI** — the `db` lane refuses without an isolated target. **54 of them are write-capable**, so pointing the lane at the shared project is not an option, and the guard must not be deleted. ⚠️ **Local Docker is blocked**: there are **236 local migration files vs 398 live ledger entries**, so replaying `supabase/migrations/` builds a schema that is *not* production's — passing tests would be false confidence. **A Supabase preview branch clones the live schema and sidesteps that gap entirely**; branching is confirmed available on this project. Paid per-branch-per-day; can be ephemeral (create in CI → run db lane → destroy). **This is the unlock for 75 dark guards.** | M, spend decision |
| 6.1a | ✅ **Make the dark lane loud** | — | Shipped: `tests/qa/unit/db-lane-coverage.test.js` prints the unrun count every CI run and fails if it grows unacknowledged. The lane runner already failed on in-lane skips; a whole lane going dark was the blind spot. | ✅ done |
