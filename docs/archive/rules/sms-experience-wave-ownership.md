...
**ARCHIVED 2026-07-31.** The sms-experience initiative is complete. Sections 1–11 below —
the ownership matrix, frozen-contract specifications, migration/css rules and the
tech-messages-v2 / messaging-transport addenda — are **history, not law**; they record what
shipped and remain the reference for the frozen contracts live code still consumes
(`§3`/`§9` shapes, `§7` artifacts). Read them on demand; they no longer auto-load.

**The live consent law moved and did NOT change.** Former `§12` (verified prior-consent
attestation) and `§13` (opt-out-only consent model) are now
[`.claude/rules/sms-consent-model.md`](../../../.claude/rules/sms-consent-model.md), verbatim
and still auto-loading, with their section numbers preserved so every existing "§12"/"§13"
