...
| **Phase 5 step 2 — money-read migration** | **AUTHORED, UNAPPLIED.** `20260807230000_office_financial_read_boundary` + rollback + 15-check static test |
...
**2. Behavioural proof for the migration** — `database-standard.md` §5b. Model on
`scripts/qa/qualify-estimate-create-boundary-local.mjs`. Must carry **per-role ALLOW and DENY
cases including the roles the change is not about** (supervisor, estimator, field_tech,
crm_partner) and a service_role case, on a disposable local stack.

**3. Owner authorizes the apply.** One Supabase behind dev and production — this is a
production change. Separate explicit yes; "do it all" does not cover it.
