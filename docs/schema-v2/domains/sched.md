...
CI DB-LANE FACTS (relevant if the owner overrides any reclassification): the db lane is now LIVE — .github/workflows/ci.yml:142-167 runs `npm run test:db:branch` against seeded qa-staging, gated by the shrink-only baseline in scripts/qa/db-lane-baseline.json (maxFailedTests: 19). Dropping get_my_appointments_today would hard-fail FIVE assertions (supabase/tests/db_foundation_p6_timezone_rpcs.test.js:103-104; supabase/tests/tech_v2_feed_upgrades.test.js:90-97, :100-104, :106-113 (two calls), :117-124) because db.rpc THROWS on non-OK — that is +5 over the baseline and the gate fails.
