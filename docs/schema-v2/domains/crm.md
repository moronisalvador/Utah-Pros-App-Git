...
- **table:contact_tags**
  - merge_contacts(p_keep_id, p_merge_id) sweeps it BY NAME — catalog/fndefs-q3-06.sql:320-321 (`DELETE FROM contact_tags WHERE contact_id = p_merge_id AND tag IN (...)` + `UPDATE contact_tags SET contact_id = p_keep_id ...`). merge_contacts is LIVE (src/components/MergeTool.jsx:101, src/components/MergeModal.jsx:62). Its supabase/tests/crm_merge_contacts_safety.test.js guard is discovered by hosted CI but currently fails in setup before its assertions, so it is not effective regression evidence yet. Independently, plpgsql resolves table names at EXECUTION, so a bare DROP TABLE makes every contact merge throw `relation "contact_tags" does not exist` at runtime with no compile-time warning. P5 must CREATE OR REPLACE merge_contacts (deleting those two lines) in the SAME migration as the DROP. Source: supabase/migrations/20260702_crm_phase0F_merge_contacts_safety.sql:125.
- **table:lead_score_factors**
  - Its intended guard, supabase/tests/crm_phase9_intelligence.test.js:57 (`db.delete('lead_score_factors', ...)`), :97 and :115 (`db.select('lead_score_factors', ...)`), is discovered by hosted CI but currently fails in setup before those assertions. Dropping the table does not have a reliable hosted regression gate until that legacy anon-era suite is converted. Independent ordering blocker: score_lead() is the sole writer — drop it in the same migration or score_lead breaks.
- **rpc:score_lead(p_lead_id uuid)**
  - supabase/tests/crm_phase9_intelligence.test.js:86 and :111 intend to call `db.rpc('score_lead', { p_lead_id: lead.id })`, but the suite currently fails in setup before those assertions. Convert the suite before treating it as a hosted drop guard. Secondary: src/lib/crmPipeline.js scoreLead() is its behavioral JS twin (referenced at :32, :97, :117) and becomes an orphan twin with no SQL counterpart if only the RPC goes.
...
TIMING FACT THAT CHANGES A MAPPER JUDGEMENT. The mapper discounted supabase/tests/ references as "test-only" because the db lane was dark. Per .claude/rules/initiative-status.md qa-staging is now live, but the raw 2026-07-31 receipt showed 46 setup errors across 44 files / 90 failed suite nodes. The CRM intelligence and merge-contact suites are discovered yet fail before their behavioral assertions, so they do not currently protect score_lead, lead_score_factors, inbound_leads.lead_score, or contact_tags from a drop. Treat those references as intended future guards until converted; rely on the independent function/view dependencies in the kill notes today.
