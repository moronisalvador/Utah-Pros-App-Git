...
| OTA | Official app exact-default-off in `nativeUpdater.js`; isolated UPR Dev canary uses crash-latched `NativeUpdateHealthGate.jsx`, generated dev-only config, signed-artifact verification, and emergency disable. Provider-capable publish/assignment/activation/rollback are absent; the retained publish choice fails before credentials or provider traffic |
