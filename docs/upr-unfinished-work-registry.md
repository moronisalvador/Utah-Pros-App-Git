...
**Draft refreshed:** 2026-07-26; mobile S1d notify RPC migration locally verified, not applied
...
| UPRF-MOB-REC-RLS-001 | CallRail recording-source direct database boundary | PARTIAL | not_started | internal + shared-database apply + owner | active | Critical / P0 residual | S1c evidence; authenticated `get_inbound_leads` and broad `inbound_leads` policies expose stored recording URLs outside the hardened proxy | mobile security + CRM DB owner | recording-source RPC/table grants, RLS, exact mobile/desktop consumers | Re-capture current grants/policies/callers without reading recording/customer contents; design one compatible RLS/RPC boundary and rollback; do not combine with S1d apply or private-media work; retire after live negative/positive role proof |
