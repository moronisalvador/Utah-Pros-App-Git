...
| UPRF-ENC-001 | Encircle managed-credential rollout | PARTIAL | landed | owner + external + verification_tail | active | High / P1 | `0a06a21`, ENC-001 | owner + unassigned rollout owner | migration/flag/candidate/runtime smoke/rotation | Migration unapplied, flag OFF, credentials unchanged; serialize apply; retire after rotation/fallback cleanup |
...
| UPRF-SEC-001 | `exec_read_sql` containment | PARTIAL | ready | owner | active | Critical / P0 | SEC-001, DB-AUTH-001 | unassigned | one migration/test/rollback; function ACL | Owner apply window; denied browser roles; service contract; dated verification |
...
| UPRF-REL-001 | migration provenance | PARTIAL | ready | internal | active | High / P0 | ARCH-001, REL-001 | unassigned | four selected source records + read-only gate | rebase after F1; no whole-commit restore; every live entry reachable/fingerprinted |
...
| UPRF-QA-001 | isolated QA foundation | MISSING | not_started | external + owner | active | High / P0 | TEST-001, ARCH-002 | unassigned | isolated target/config/seeds/reset/CI refusal | F3a/b/c acceptance; retire into maintained QA operations |
...
| UPRF-PUB-001 | public form boundary | PARTIAL | not_started | internal | active | Critical / P0 | PUB-001 | unassigned | form Worker/RPC/tests | service-only transaction and direct denial; serialize shared docs/tests |
...
|---|---|---|---|---:|---|---|---|
| ENC-001 | Encircle managed integration | PARTIAL | verification_tail | P1 | `0a06a21` on `origin/dev`; owner-reported CI/staging pass; managed migration unapplied, flag OFF, credentials unchanged | Apply/test; owner-only flag; candidate activation; Pages/MCP smoke; rotation/fallback cleanup | Owner + rollout owner |
| SEC-001 | `exec_read_sql` containment | PARTIAL | ready | P0 | Current live query; `20260627_exec_read_sql.sql:22-57`; `20260708_dbf_p3_anon_rpc_revoke.sql:160-161` | Owner apply window; revoke browser roles; negative tests | Security DB owner |
...
| REL-001 | Migration provenance | PARTIAL | planned | P0 | Four live names absent from checkout | Compare exact live bodies; restore reviewed source; add gate | Release/DB owner |
...
| QA-002 | Isolated Supabase and test data | MISSING | blocked_external | P0 | Shared DB; current no QA/test tables; `testing-and-deployment.md:38-40` | Owner budget/project/reset/roles; migration-from-zero | QA/DB owner |
| QA-003 | Split unit/DB/browser/accessibility lanes | MISSING | blocked_internal | P1 | `package.json` has build/lint/Vitest only; CI nonblocking lint/bundle | QA-002; zero unexpected skips; Playwright/axe/device matrix | QA owner |
...
| PUB-001 | Public form RPC bypass | PARTIAL | planned | P0 | `crm_phase10_form_rpcs.sql:177-228,280-330`; Worker anti-abuse path | Service-only transaction; server-derived evidence; direct denial | Public boundary owner |
