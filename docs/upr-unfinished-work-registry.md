...
| MOB-SEC-014 | Mobile trusted authorization boundary | PARTIAL | s1f_bell_rpc_ready_for_apply | P0 | R0→S1f evidence; QBO/CallRail/notify HTTP source gates plus separate unapplied S1d notify, S1e recording-source, and S1f bell-emission migrations exist; wider mobile RPC/direct policies remain | Integrate and apply/verify S1d, S1e, and S1f only in separate owner windows; then shared identity/device/notification-read/merge/public-signing and route-family RPC/direct-policy slices; QBO telemetry/RLS remain separately tracked | Mobile security owner |
