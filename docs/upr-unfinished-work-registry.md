...
**Draft refreshed:** 2026-07-26; mobile S1e recording-source migration locally verified, not applied
...
| UPRF-QA-001 | isolated QA foundation | PARTIAL | p1_verified_p2a_blocked | internal complete; external + owner tail | active | High / P0 | TEST-001, ARCH-002, `isolated-qa-figma-internal-foundation-2026-07-23.md` | owner + QA/DB owner | delivered P1 config/guards/fixtures/CI; P2a local runtime/config/seeds/reset; P2b hosted target | Preserve zero-skip/refusal gates; next open governed local Supabase runtime/config for P2a or provision dedicated hosted QA for P2b; retire into maintained QA operations |
...
| UPRF-MOB-001 | Mobile PWA/Capacitor production readiness | PARTIAL | s1d_notify_rpc_ready_for_apply | internal + owner + external | active | Critical / P0 | 2026-07 mobile audit: 2 P0, 21 P1, 14 P2; `mobile-production-readiness-roadmap.md`; R0 plus S1b/S1c/S1d dated evidence; S1a commits `66182f3..e42edb5`; S1d migration `20260726110000` | owner + mobile wave owner | PWA, tech/admin mobile, durable device state, workers/RPC/RLS/Storage, Capacitor/iOS, release/device proof | Apply S1d only in its separate owner-authorized window; next source slice is recording-source RLS, followed separately by shared identity/device/bell/merge/public-signing and route-family RPC/RLS; QBO residuals remain separately owned below; private media remains a separate compatibility/apply wave; no live apply/deploy/signing by implication; retire after fresh qualification audit |
...
| UPRF-MOB-BELL-RPC-001 | Direct `create_notification` bell emission | PARTIAL | not_started | internal + shared-database apply + owner | active | High / P1 residual | S1c/S1d evidence; authenticated definer remains directly executable outside `/api/notify` | mobile security + notification owner | `create_notification` ACL/body/callers only | Inventory exact live callers and recipient reconstruction, then author a separate least-privilege boundary with browser-denial and caller-compatibility tests; do not mix into `notify_emit`; retire after applied proof |
...
| MOB-SEC-014 | Mobile trusted authorization boundary | PARTIAL | s1d_notify_rpc_ready_for_apply | P0 | R0, S1b/S1c/S1d evidence; nine QBO Worker routes plus CallRail recording/notify HTTP locally contained; `notify_emit` migration/rollback/tests authored but unapplied; recording-source reads, `create_notification`, 84 client-reachable mobile RPCs (82 authenticated-tech + two public-signing; four anon/three PUBLIC) and broad direct policies remain | Apply/verify UPRF-MOB-NOTIFY-001 separately; next author UPRF-MOB-REC-RLS-001; then shared identity/device/bell/merge/public-signing and route-family RPC/direct-policy slices; QBO telemetry/RLS stay separately tracked as UPRF-MOB-QBO-AUD-001/UPRF-MOB-QBO-RLS-001 | Mobile security owner |
