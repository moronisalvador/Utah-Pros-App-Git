...
**Draft refreshed:** 2026-07-26; mobile S1g notification-read migration and local runner bridge verified, not applied
...
| UPRF-QA-001 | isolated QA foundation | PARTIAL | p1_verified_p2a_blocked | internal complete; external + owner tail | active | High / P0 | TEST-001, ARCH-002, `isolated-qa-figma-internal-foundation-2026-07-23.md`; S1g local-only pgTAP bridge | owner + QA/DB owner | delivered P1 config/guards/fixtures/CI; S1g exact SQL is wired but governed current-live-compatible local runtime/config/seeds/reset remains P2a; P2b hosted target | Preserve zero-skip/refusal gates; next open governed local Supabase runtime/config for P2a or provision dedicated hosted QA for P2b; retire into maintained QA operations |
| UPRF-MNY-001 | QBO/Stripe safety | PARTIAL | authorization_slice_verified | external + internal tail | active | Critical / P0 | AUTH-001/002 and COR-001 closed; COR-002/003 remain; `money-worker-hardening-2026-07-23.md` | money owner | separate Workers/tests; shared MT helper | retain role/provider-never-called/date/request-key tests; next durable QBO attempt/recovery migration after Intuit sandbox contract; Stripe reuse/expiry/concurrency separately |
...
| UPRF-MOB-001 | Mobile PWA/Capacitor production readiness | PARTIAL | s1g_notification_reads_ready_for_apply | internal + owner + external | active | Critical / P0 | 2026-07 mobile audit: 2 P0, 21 P1, 14 P2; `mobile-production-readiness-roadmap.md`; R0→S1g dated evidence; checksum-pinned `docs/mobile/s1d-s1g-database-apply-runbook.md`; separate S1d/S1e/S1f/S1g unapplied migrations | owner + mobile wave owner | PWA, tech/admin mobile, durable device state, workers/RPC/RLS/Storage, Capacitor/iOS, release/device proof | Keep S1d/S1e/S1f/S1g applies separate; next source slice is shared identity/device/preferences, followed by merge, public signing, and route-family RPC/RLS; QBO residuals and private media remain separately owned; no live apply/deploy/signing by implication; retire after fresh qualification audit |
...
| UPRF-MOB-REC-RLS-001 | CallRail recording-source direct database boundary | PARTIAL | ready_for_apply | compatible Worker deploy + shared-database apply + owner | active | Critical / P0 residual | S1e evidence; migration/rollback `20260726183409`; raw URL moves to forced-RLS service-only source table | mobile security + CRM DB owner | source table/trigger, `get_inbound_leads`, direct grants/RLS, proxy/transcription consumers | Deploy compatible Workers first; recapture catalog-only drift, apply separately from S1d, prove browser denial/marker shape/service reads without selecting URL values or calling providers, refresh provenance/advisors; retire after live negative/positive proof |
...
| UPRF-MOB-NOTIF-READ-001 | Notification read/mark recipient and Realtime boundary | PARTIAL | source_ready_for_apply | internal + shared-database apply + owner | active | Critical / P0 residual | S1g evidence; four same-signature RPC replacements, private broadcast receipts, scoped SELECT policy, exact rollback/catalog checks, local pgTAP bridge, and synthetic PostgreSQL behavior execution; live BOLA/shared-read policy remains until apply | mobile security + notification/DB owner | bell list/count/mark RPCs, `notifications` RLS/ACL/Realtime, private receipt table | Integrate reviewed S1g; recapture exact catalog drift; apply only S1g in its own window; prove active-internal own/broadcast allow plus foreign/inactive/external/unmapped denial and two-session Realtime/read isolation; refresh provenance/advisors; never batch with S1d/S1e/S1f |
...
| COR-002 | QBO captured-but-unrecorded/idempotency recovery | PARTIAL | blocked_external_internal | P0 | stable required client request ID passed to Intuit; cents/balance/actor tests; no durable pre-provider attempt | Intuit sandbox semantics; service-only attempt ledger; injected provider-success/local-failure retry; reconciliation job/runbook | Money owner |
...
| QA-002 | Isolated Supabase and test data | PARTIAL refusal / MISSING runtime and target | blocked_external | P0 | local-only runner rejects shared/missing targets; `isolated-qa-figma-internal-foundation-2026-07-23.md` | Governed local config/runtime/seed/roles for P2a; owner budget/project/region/retention for P2b; shared DB remains forbidden | QA/DB owner |
| QA-003 | Split unit/DB/browser/accessibility lanes | HAVE internal foundation | verification_tail | P1 | partitioned Vitest runners; Playwright/axe desktop+390 synthetic matrix; CI; zero unexpected skips | Add isolated DB execution, pinned-Linux visual baselines, representative authenticated UPR journeys, zoom/reflow and real-device/manual evidence | QA owner |
| GOV-001 | Canonical skills/agents source and adapters | PARTIAL pilots implemented | verification_tail | P1 | `tooling/capabilities.json`; neutral generated adapters/tests/reviewer parity; `.claude/mobile-readiness-codex-adapters.json`; mobile adapter tests | Runtime smoke in fresh Claude/Codex sessions; inventory and migrate remaining UPR-authored entrypoints deliberately; retain mobile adapters until a governed neutral successor covers them | Governance owner |
...
| MOB-SEC-014 | Mobile trusted authorization boundary | PARTIAL | s1g_notification_reads_ready_for_apply | P0 | R0→S1g evidence; QBO/CallRail/notify HTTP source gates plus separate unapplied S1d notify, S1e recording-source, S1f bell-emission, and S1g bell-read/RLS migrations exist; `docs/mobile/s1d-s1g-database-apply-runbook.md` pins four independent windows; wider mobile RPC/direct policies remain | Integrate and apply/verify S1d, S1e, S1f, and S1g only in separate owner windows; then shared identity/device/preferences, merge, public-signing and route-family RPC/direct-policy slices; QBO telemetry/RLS remain separately tracked | Mobile security owner |
...
| APP-001 | App Store/Xcode/ASC completion | PARTIAL | blocked_external | P1 | manual-only workflow validity repaired/tested 2026-07-23; unsigned generic and iPhone 17 Pro/iOS 26.5 simulator build/install/launch evidence 2026-07-25; `app-store-readiness-roadmap.md` §6 | Apple enrollment/signing secrets; privacy manifest; signed archive/device/TestFlight; screenshots, demo account, ASC entry | Owner |
