...
**Draft refreshed:** 2026-07-23 on `dev`; P1 isolated-QA/Figma internal close-out. Native iOS
planning row added 2026-07-25 and Apple Field Pro direction amended 2026-07-26; other rows were not
broadly re-audited by those planning updates.
...
| UPRF-NIOS-001 | parallel native Swift iOS client | HAVE plan | current_client_hardening_gate_open | owner review + current-client hardening + Mac/Xcode + QA | active | High / P1 | `docs/native-ios/README.md`, ADRs 0001/0002/0003, `NIOS-H`, owner decisions 2026-07-25/26 | owner + current-client hardening owner + native orchestrator | current audit/hardening handoff first; future Swift paths only after Gate H + Phase 0 + implementation authority | Reuse completed mobile audit/finding IDs; record `NIOS-H: READY`; then run Mac Discovery A/B for device/field/native-translation/architecture/first-slice/QA gates; preserve PWA/Capacitor through controlled native cutover |
...
| NIOS-001 | Parallel native Swift iOS application | HAVE plan | hardening_precondition_open | P1 | `docs/native-ios/`; accepted ADRs 0001/0002/0003; Apple Field Pro evolution approved; `NIOS-H`; base `90b265e` | Finish current-client hardening from existing audit state and record Gate H; then complete Mac field/device/native-translation workshop, architecture/first-slice and governed-QA gates; do not modify or retire Capacitor during native planning | Owner + current-client hardening owner + native orchestrator |
...
| APP-001 | App Store/Xcode/ASC completion | PARTIAL | blocked_external | P1 | manual-only workflow validity repaired/tested 2026-07-23; `ios-workflow-validation-2026-07-23.md`; `app-store-readiness-roadmap.md` §6 | Apple enrollment/signing secrets; owner Xcode archive/TestFlight; screenshots, demo account, ASC entry | Owner |
