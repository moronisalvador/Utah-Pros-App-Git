...
- **Severity:** Critical
- **Confidence:** confirmed
- **Evidence:** `supabase/migrations/20260627_exec_read_sql.sql:1-20`; `supabase/migrations/20260627_exec_read_sql.sql:22-58`; `supabase/migrations/20260627_exec_read_sql.sql:60-64`; `supabase/migrations/20260708_dbf_p3_anon_rpc_revoke.sql:158-161`; `docs/audit/2026-07/evidence/live-supabase.md:100-114`.
...
- **Observed behavior:** `exec_read_sql(text)` runs as `SECURITY DEFINER`, dynamically executes caller-provided `SELECT`/`WITH` text and has no employee/role/allowlist check. The original migration says service-role only, but a later migration and the live grants allow `authenticated`. Read-only mode prevents mutation; it does not preserve RLS or restrict selected objects.
...
- **Recommended remediation:** immediately revoke EXECUTE from `authenticated` and `PUBLIC`, retain service-role only if the MCP genuinely requires it, and preferably move the helper into a non-exposed schema. Add an explicit migration that corrects the contradictory grant/comment and inventory logs for prior direct calls if an audit source exists. Do not attempt to make free-form SQL safe with string parsing.
- **Regression test / verification:** catalog test proves only `service_role` (or a narrower dedicated server role) can execute; anon and authenticated RPC calls return permission denied; a denied test attempts reads from `public`, `auth` and credential tables. Re-run the security advisor.
