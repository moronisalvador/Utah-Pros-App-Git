...
`UPDATE ... RETURNING` result is the only authority to process a row. The exact-source migration
and service-only live readback passed under ledger version `20260724051500`; Worker deployment and
one stale-event recovery are still required before inbound MMS recovery is closed.
