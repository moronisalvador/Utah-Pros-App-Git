...
> **◐ YELLOW APPLIED · RED STAGED 2026-07-08** (branch `claude/phase-p4-db-integrity-xpr8au`). Report: `docs/db-foundation-p4-orphan-report.md`; gate `supabase/tests/db_foundation_p4_data_integrity.{sql,test.js}` green; `migration-safety-checker` clean. **Applied live (YELLOW):** `notifications.job_id`→`jobs` FK, `job_time_entries` duration CHECKs, partial-UNIQUE on `forms.encircle_note_id` + `google_calendar_links.google_event_id`. **Staged, awaiting owner OK (RED):** external-ID dedup repair (4 non-canonical claims + 1 stray contact, verified via Encircle/QBO) + partial-UNIQUE on `claims.encircle_claim_id`/`contacts.qbo_customer_id` (supersedes the plain claims index). **Key finding:** `invoices.qbo_invoice_id`/`payments.qbo_payment_id` "dups" are legitimate combined billing (QBO total = sum of the two per-job UPR rows) — NOT repaired, NOT constrained; `invoices` `qbo_invoice_id=4274` is a genuine anomaly flagged for owner. Apply the RED migrations in a window NOT overlapping P3.
...
> **⚙️ HALF 1 SHIPPED 2026-07-08** (branch `claude/phase-p5-perf-indexes-j65h18`; gauntlet green). **Covering-index half applied + verified live:** 7 hot-path FK covering indexes (`idx_jobs_lead_tech_id`, `idx_invoices_estimate_id`, `idx_estimates_converted_invoice_id`, `idx_job_documents_sign_request_id`, `idx_sign_requests_contact_id`, `idx_job_time_entries_continued_from`, `idx_conversation_participants_contact_id` — all `indisvalid`). A deliberately tight subset of the 108 unindexed FKs: **excluded** the employee audit columns (`created_by`/`updated_by`/`recorded_by`/… — never filtered, parent never DELETEd) and the zero-row flag-gated CRM/form/sequence tables (`page:crm` closed → not hot-path). Migration `20260708_dbf_p5_fk_covering_indexes.sql` (YELLOW, additive, rollback in header). **DROP-unused/duplicate half DEFERRED — blocked on P6 (no open PR yet):** it needs P6's view/RPC definitions to build the exclusion list and a fresh `idx_scan` re-verify, and is RED-tier (owner OK). Ship it as a separate follow-up once P6 merges.
...
> **✅ SHIPPED 2026-07-08** (session-assigned branch `claude/phase-p6-reporting-foundation-ujgv7l`). Applied + verified live on the shared Supabase. **① Reporting-views layer** — 5 tracked views (`rv_jobs`/`rv_invoices`/`rv_payments`/`rv_leads`/`rv_time_entries`), all `WITH (security_invoker = true)`, `REVOKE ALL FROM PUBLIC, anon` + `GRANT SELECT TO authenticated, service_role` (`20260708_dbf_p6_reporting_views.sql`; verified anon-denied live). **② Timezone RPC body-replaces** — 8 live RPCs swapped naive `CURRENT_DATE` → `public.mt_today()` (DB session TZ is UTC — verified — so this was a real Denver fix, not a no-op); signatures + RETURNS shapes byte-identical (drift-dumped via `pg_get_functiondef` first — `add_custom_schedule_phase`/`get_payroll_summary`/`get_timesheet_entries` were not in the repo); grants preserved byte-identical (anon retained — P3 owns anon closure), `REVOKE EXECUTE … FROM PUBLIC` per fn (managed-Supabase trap) (`20260708_dbf_p6_timezone_rpc_bodies.sql`). **Disclosed rule amendment (manifest §3):** `get_call_volume`/`get_conversion_trend` (CRM Phase-9 frozen) + `get_my_appointments_today`/`get_assigned_tasks`/`get_stalled_materials_for_employee` (tech-v2 frozen) were body-only-replaced for the timezone fix; their existing backward-compat tests assert return shape only and stay green. **③ event_type registry** section added to `UPR-Web-Context.md`. Return-shape guards: `supabase/tests/db_foundation_p6_{reporting_views,timezone_rpcs}.test.js`.
