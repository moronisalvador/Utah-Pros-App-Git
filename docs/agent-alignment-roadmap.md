...
| "An unescaped `[` in a `paths:` glob silently matches nothing" | Bracket classes **work**. ~~Brace groups match nothing~~ — **also refuted, 2026-07-26**: braces load on both installed builds; only the ~1,000-expansion budget fails, and only on 2.1.219 | Both halves refuted. The authoring rule is **budget-aware**, not brace-free (§4c of the L2 evidence) |
...
> - [ ] **GLOB LINTER SOUND BEFORE IT IS TRUSTED.** `node scripts/agents/verify-rule-globs.mjs --self-test` reproduces the loader's pipeline as read from the bundle — strip a trailing `/**` from each entry, drop empties, and if every survivor is `**` treat the rule as UNCONDITIONAL, then match with gitignore semantics against repo-root-relative paths — and proves the behaviours measured directly — **CORRECTED 2026-07-26 (`agent-alignment-l2-evidence.md` §4c): `src/**/*.{js,jsx}` DOES match `src/pages/Foo.jsx` and `src/pages/Foo.js`.** The old spec here demanded the linter prove the opposite, which would have encoded a non-existent loader behaviour. The real failure mode to assert is the **expansion budget** — brace groups multiplying to ≥ ~1,000 patterns silently never match on 2.1.219 (512 loads, 1024 does not; 2.1.85 does not enforce it at all). Also: `src/pages/**` matches `src/pages/Foo.jsx` and `src/pages/tech/Bar.jsx` but not `src/pagesX/Y.jsx`; `functions/api/callrail-*.js` matches `callrail-connect.js` but not `twilio-webhook.js`; and `src/pages/[A-Z]*.jsx` **does** match `src/pages/Admin.jsx` — **refuting** the inherited claim that an unescaped `[` matches nothing. Brace-free, not bracket-free, is the rule
