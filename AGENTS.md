...
The bounded mobile-readiness pilot remains separately declared in
`.claude/mobile-readiness-codex-adapters.json`; update its canonical `.claude` sources and run
`npm run generate:mobile-codex`, never hand-edit its generated adapters.
...
Use one primary and no more than three simultaneous subagents. The project-scoped mobile Codex roles
are generated from canonical `.claude/agents/mobile-readiness-*.md` files through the bounded legacy
manifest; run `npm run generate:mobile-codex` and never hand-edit generated adapters. Follow the
ownership manifest's single-writer/shared-hotspot rules.
