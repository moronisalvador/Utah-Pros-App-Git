-- ════════════════════════════════════════════════
-- MIGRATION: 20260724195802_accept_frozen_callrail_mms_media_shape
-- Phase: CallRail production-readiness remediation
-- ════════════════════════════════════════════════
--
-- WHAT THIS DOES (plain language):
--   Keeps the outbound CallRail confirmation checks from the prior migration,
--   while accepting both historical ways UPR stored an image list. Every image
--   must still point to UPR's private outbound attachment area.
--
...
-- ════════════════════════════════════════════════
-- ROLLBACK:
--   Run supabase/rollbacks/20260724195802_accept_frozen_callrail_mms_media_shape.rollback.sql
--   to restore the exact prior function body and grants.
-- ════════════════════════════════════════════════
...
BEGIN
  ...
  IF p_attempt_id IS NULL THEN
    ...
    v_existing_media := COALESCE(v_existing_message.media_urls, '[]'::jsonb);
    ...
    IF v_event.message_type = 'mms'
       AND (
         jsonb_typeof(v_existing_media) <> 'array'
         OR jsonb_array_length(v_existing_media) = 0
         OR EXISTS (
           SELECT 1
           FROM jsonb_array_elements(v_existing_media) AS media(item)
...
COMMENT ON FUNCTION public.project_callrail_outbound_event(uuid, uuid) IS
  'Service-role-only projection of one CallRail message.sent event; channel identity and private outbound MMS ownership must match before confirmation, including the frozen canonical JSON-string media shape.';
