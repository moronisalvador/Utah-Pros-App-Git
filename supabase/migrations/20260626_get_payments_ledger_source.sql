-- 20260626_get_payments_ledger_source.sql
-- Add `source` (manual | qbo | stripe) to the payments-ledger RPC so the Collections
-- → Payments tab can show an "Online · QBO" pill. Additive: same query, one more column.
-- DROP first because CREATE OR REPLACE can't add a column to a function's RETURNS TABLE.

DROP FUNCTION IF EXISTS get_payments_ledger(int);

CREATE FUNCTION get_payments_ledger(p_limit int DEFAULT 500)
RETURNS TABLE (
  ...
  is_deductible boolean, created_at timestamptz, source text,
  qbo_payment_id text, qbo_synced_at timestamptz, qbo_sync_error text,
  invoice_id uuid, invoice_number text, qbo_doc_number text,
