...
--   Historical policy objects remain as fail-closed catalog records, and
--   browser roles have no table privilege through which to reach the queue.
--
-- ADDITIVE-ONLY / attribute-only / etc.:
--   Authorization-only grant/policy closure plus a fail-closed replacement of the
  ...
  LOOP
    IF v_policy.policyname NOT IN ('allow_anon_read_scheduled_messages', 'allow_anon_insert_scheduled_messages', 'allow_authenticated_scheduled_messages')
       ...
       OR (v_policy.policyname = 'allow_anon_read_scheduled_messages'
           AND (v_policy.cmd <> 'SELECT' OR regexp_replace(COALESCE(v_policy.qual, ''), '\s+', '', 'g') <> 'false' OR v_policy.with_check IS NOT NULL))
       OR (v_policy.policyname = 'allow_anon_insert_scheduled_messages'
           AND (v_policy.cmd <> 'INSERT' OR v_policy.qual IS NOT NULL OR regexp_replace(COALESCE(v_policy.with_check, ''), '\s+', '', 'g') <> 'false'))
       OR (v_policy.policyname = 'allow_authenticated_scheduled_messages'
           AND (v_policy.cmd <> 'ALL' OR regexp_replace(COALESCE(v_policy.qual, ''), '\s+', '', 'g') <> 'false' OR regexp_replace(COALESCE(v_policy.with_check, ''), '\s+', '', 'g') <> 'false')) THEN
...
-- Both hosted catalogs were read-only verified on 2026-07-31: the policy
-- objects are authenticated-only. Compatibility already changed every
-- predicate to FALSE; reassert that fail-closed state beside the ACL boundary.
...
ALTER POLICY allow_anon_read_scheduled_messages
  ON public.scheduled_messages TO authenticated USING (false);
ALTER POLICY allow_anon_insert_scheduled_messages
  ON public.scheduled_messages TO authenticated WITH CHECK (false);
ALTER POLICY allow_authenticated_scheduled_messages
  ON public.scheduled_messages TO authenticated
...
-- Preserve the frozen signature and historical grants for stale callers, but
-- return FALSE without touching a row. The v2 fenced workflow is the only
-- service path that can claim a scheduled delivery.
  ...
  LOOP
    IF v_policy.policyname NOT IN (
       ...
       OR (
         ...
         AND (
           v_policy.cmd <> 'SELECT'
           OR regexp_replace(COALESCE(v_policy.qual, ''), '\s+', '', 'g') <> 'false'
       ...
       OR (
         ...
         AND (
           ...
           OR regexp_replace(COALESCE(v_policy.with_check, ''), '\s+', '', 'g') <> 'false'
         )
       )
       OR (
      ...
      RAISE EXCEPTION
        'scheduled delivery enforcement: fail-closed policy postcondition failed';
    END IF;
  END LOOP;
