-- ════════════════════════════════════════════════
-- MIGRATION: 20260725173000_fix_callrail_keyword_consent_ambiguity
-- Phase: P0 — inbound STOP was never being honoured
-- ════════════════════════════════════════════════
--
-- WHAT THIS DOES (plain language):
--   Fixes the bug that stopped "STOP" from working. When a customer texted
--   STOP, the code that records the opt-out crashed instead of running, so the
--   person stayed subscribed. Every first message we send tells people to reply
--   STOP to unsubscribe, so this had to be fixed before any further texting.
--
-- THE BUG (found by a live owner test, 2026-07-25 17:13Z):
--   project_callrail_inbound_event is declared RETURNS TABLE(..., contact_id
--   uuid, ...). A RETURNS TABLE output column is also a PL/pgSQL variable, so
--   the bare `contact_id` inside
--     ON CONFLICT (provider_event_id, contact_id, event_type)
--       WHERE provider_event_id IS NOT NULL
--   is ambiguous between that variable and the sms_consent_log column. Postgres
--   raises 42702 "column reference \"contact_id\" is ambiguous", the whole
--   projection aborts, and the event parks in processing_state='retryable'
--   forever — retrying and failing every five minutes.
--
--   Why it hid for so long: BOTH occurrences sit in branches that ordinary
--   traffic never enters. One is inside `IF v_keyword IS NOT NULL` (only
--   STOP/START/HELP), the other inside `IF v_keyword IS NULL` on the
--   brand-new-contact path. 14 normal inbound texts projected fine because they
--   hit neither. sms_consent_log has never held a single stop event.
--
-- THE FIX:
--   Add `#variable_conflict use_column` to the function body. PL/pgSQL's
--   default is `error`, so today EVERY ambiguous name aborts; switching to
--   use_column resolves them to the table column. It cannot change a statement
--   that works now — a working statement had no ambiguity to resolve. Body-only.
--
-- ADDITIVE-ONLY:
--   One function-body-only CREATE OR REPLACE. Signature, argument list, RETURNS
--   TABLE shape, language, SECURITY INVOKER and search_path are byte-identical
--   to the live definition (verified: live body md5 a6ff6de0591ffb08a403a89e10cbfd7d
--   matches this repo's 20260723215926 source exactly before patching). No table,
--   column, policy, grant or data changes.
--
-- ════════════════════════════════════════════════
-- ROLLBACK:
--   supabase/rollbacks/20260725173000_fix_callrail_keyword_consent_ambiguity.rollback.sql
--   restores the identical body WITHOUT the directive. Note that rolling back
--   re-breaks inbound STOP — do not roll back while messaging is live.
-- ════════════════════════════════════════════════
