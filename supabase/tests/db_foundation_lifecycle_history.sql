...
DO $$
DECLARE
  cid uuid; old_s text; h0 int; h1 int; h2 int; lat_from text; lat_to text;
  iid uuid; oi_s text; g0 int; g1 int; g2 int;
...
BEGIN
  ...
  UPDATE claims SET status = COALESCE(old_s, 'x') || '_DBF_TEST' WHERE id = cid;   -- real change → fire
  ...
  IF lat_to <> COALESCE(old_s,'x') || '_DBF_TEST' OR lat_from IS DISTINCT FROM old_s THEN
  ...
  UPDATE invoices SET status = COALESCE(oi_s, 'x') || '_DBF_TEST' WHERE id = iid;   -- real change → fire
