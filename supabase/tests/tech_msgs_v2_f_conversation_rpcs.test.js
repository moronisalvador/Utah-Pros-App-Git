 ...
 *   Proves the two Tech Messages v2 Foundation database helpers reject a logged-OUT
 *   (anon) visitor. get_tech_conversations remains available to authenticated
 *   callers; find_or_create_conversation is now service-role-only behind the
 *   capability-checked /api/message-conversations Worker. The shape/cursor/
 ...
 *   - Both RPCs reject anon. find_or_create additionally revokes authenticated
 *     execution after migration 20260724173000. It is called with a random uuid so
 *     that even if the guard ever regressed, no real thread is created.
 * ════════════════════════════════════════════════
 */
...
describe.skipIf(!hasCreds)('Tech Messages v2 F-M RPC least-privilege (integration)', () => {
  ...
  it('anon CANNOT execute find_or_create_conversation (service_role only)', async () => {
    await expect(
