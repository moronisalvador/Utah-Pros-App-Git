/**
 * ════════════════════════════════════════════════
 * FILE: qualify-estimate-create-boundary-local.mjs
 * ════════════════════════════════════════════════
 *
 ...
 *   them, applies the estimate-create permission change, checks it behaves
 *   correctly, undoes it, checks the undo really re-opened the hole it closed,
 *   applies it again, and then deletes the whole throwaway copy. Nothing it
 *   does can reach the real Utah Pros database.
 *
 * DEPENDS ON:
 ...
 *              predecessors named below, the paired 20260805020000
...
const CACHE_ROOT = path.join(os.homedir(), '.cache', 'upr-estimate-create-boundary-local');
const CONTAINER_ROOT = '/tmp/upr-estimate-create-boundary-local';

const MIGRATION = 'supabase/migrations/20260805020000_estimate_create_rpc_billing_boundary.sql';
const ROLLBACK = 'supabase/rollbacks/20260805020000_estimate_create_rpc_billing_boundary.rollback.sql';
const PROOF = 'supabase/tests/estimate_create_rpc_billing_boundary.test.sql';
const BASELINE = 'db/baseline/schema.sql';

// This migration REPLACES two function bodies and CONSUMES public.billing_edit_access(),
// so the baseline alone is not a truthful predecessor — db/baseline/schema.sql predates
// that helper entirely and the proof would fail at `function billing_edit_access() does
// not exist`. Worse, applying it on the bare baseline would "prove" the boundary against
// a role list production has not had since 2026-08-05.
//
// The exact predecessors live in production, in ledger order — the same four the
// billing-boundary qualifier uses, plus the boundary migration itself, which is what
// widens billing_edit_access() to ('admin','office','project_manager'):
...
DECLARE
  v_body text;
BEGIN
  -- 1. The rollback must REMOVE the guard from both estimate-create RPCs. That
  --    re-opens the hole on purpose; if the guard survived, the rollback is not
  --    a rollback and the operator has no way back.
  FOR v_body IN
    SELECT pg_get_functiondef(p.oid)
      FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
     WHERE n.nspname = 'public'
       AND p.proname IN ('create_estimate_for_contact', 'create_estimate_for_job')
  LOOP
    IF v_body LIKE '%billing_edit_access()%' THEN
      RAISE EXCEPTION 'rollback left an estimate-create RPC still gated on billing_edit_access()';
    END IF;
    IF v_body NOT LIKE '%SECURITY DEFINER%' THEN
      RAISE EXCEPTION 'rollback changed the SECURITY DEFINER posture of an estimate-create RPC';
    END IF;
  END LOOP;

  -- 2. Both functions must still EXIST with their deployed grants — a rollback
  --    that drops or un-grants them breaks NewEstimateModal outright.
  IF (SELECT count(*) FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
       WHERE n.nspname = 'public'
         AND p.proname IN ('create_estimate_for_contact', 'create_estimate_for_job')) <> 2 THEN
    RAISE EXCEPTION 'rollback did not leave both estimate-create RPCs in place';
  END IF;
  IF EXISTS (SELECT 1 FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
    ...
    RAISE EXCEPTION 'rollback left an estimate-create RPC with the wrong EXECUTE grants';
  END IF;

  -- 3. This migration does NOT own billing_edit_access() or any policy. Rolling
  --    it back must leave the widened predicate and the estimates policies that
  --    depend on it exactly as the predecessor left them.
...
function runCycle(context, ports) {
  ...
  try {
    ...
    psql(context, container, 'postgres', ['-f', base(MIGRATION)]);
    psql(context, container, 'postgres', ['-f', base(PROOF)], { isolated: true });
    process.stdout.write('estimate-create-boundary local qualification cycle passed.\n');
  } finally {
    const errors = [];
    if (started) {
...
export function main(argv = process.argv.slice(2)) {
  ...
  process.stdout.write(`${JSON.stringify({
    schema: 'upr-estimate-create-boundary-local-qualification-v1',
    commit_sha: commit,
