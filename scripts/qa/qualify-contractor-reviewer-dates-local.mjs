/**
 * ════════════════════════════════════════════════
 * FILE: qualify-contractor-reviewer-dates-local.mjs
 * ════════════════════════════════════════════════
 *
 * WHAT THIS DOES (plain language):
 *   Builds a throwaway copy of the database on this machine, loads the saved
 *   schema, replays the five real contractor-compliance predecessors in the
 *   order production applied them, applies the change that lets a contractor
 *   send an insurance certificate without typing its dates, checks that it
 *   actually can arrive undated and still cannot be ACCEPTED undated, undoes it,
 *   checks the undo really re-closed what it opened, applies it again, and then
...
const CACHE_ROOT = path.join(os.homedir(), '.cache', 'upr-contractor-reviewer-dates-local');
const CONTAINER_ROOT = '/tmp/upr-contractor-reviewer-dates-local';

const NAME = '20260819010000_contractor_compliance_reviewer_supplied_dates';
const MIGRATION = `supabase/migrations/${NAME}.sql`;
const ROLLBACK = `supabase/rollbacks/${NAME}.rollback.sql`;
const PROOF = 'supabase/tests/contractor_compliance_reviewer_supplied_dates.test.sql';
const ROLLBACK_PROOF = 'supabase/tests/contractor_compliance_reviewer_supplied_dates.rollback.test.sql';
const BASELINE = 'db/baseline/schema.sql';

// db/baseline/schema.sql is a 2026-07-28 production dump and the contractor
// compliance module applied on 2026-08-03, so the baseline contains NONE of it —
// zero contractor_compliance objects. Applying this migration on the bare
// baseline would fail at `relation contractor_compliance_documents does not
// exist`, and the drift guard would never even run.
//
// The exact predecessors live in production, in ledger order:
//   20260803220653  contractor_compliance_foundation             -> the tables, the date CHECK
//                                                                   this migration replaces, the
//                                                                   4-argument review function,
//                                                                   and continuous_coverage_end()
//   20260803220656  contractor_annual_insurance_audits
//   20260803220659  contractor_w9_provider_handoffs
//   20260803220704  contractor_compliance_read_auth_fail_closed
//   20260803220711  contractor_compliance_fk_indexes
//
// The last four are additive and touch none of the objects here; they are
// carried for lineage fidelity, so the drift guard is checked against the same
...
const PREDECESSORS = Object.freeze([
  ...
  'supabase/migrations/20260803145000_contractor_annual_insurance_audits.sql',
  'supabase/migrations/20260803145100_contractor_w9_provider_handoffs.sql',
  'supabase/migrations/20260803220000_contractor_compliance_read_auth_fail_closed.sql',
  'supabase/migrations/20260803220100_contractor_compliance_fk_indexes.sql',
]);
...
function createNetwork(context) {
  const name = `upr-cc-dates-${process.pid}-${randomUUID().slice(0, 8)}`;
...
function runCycle(context, ports) {
  ...
  try {
    ...
    // The contractor-compliance foundation schedules its daily reminder poll with
    // cron.schedule(). The Supabase local image ships pg_cron but does not enable
    // it, so this installs the REAL extension rather than shimming the call away —
    // a stub would let a predecessor "apply" without its scheduler existing.
    psql(context, container, 'postgres', ['-c', 'CREATE EXTENSION IF NOT EXISTS pg_cron;']);
    psql(context, container, 'postgres', ['-c', "DO $$ BEGIN IF to_regclass('cron.job') IS NULL THEN RAISE EXCEPTION 'local pg_cron unavailable'; END IF; END $$;"]);
    ...
    psql(context, container, 'postgres', ['-f', base(MIGRATION)]);
    psql(context, container, 'postgres', ['-f', base(PROOF)], { isolated: true });
    process.stdout.write('contractor reviewer-dates local qualification cycle passed.\n');
  } finally {
    const errors = [];
    if (started) {
...
export function main(argv = process.argv.slice(2)) {
  ...
  process.stdout.write(`${JSON.stringify({
    schema: 'upr-contractor-reviewer-dates-local-qualification-v1',
    commit_sha: commit,
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  try { main(); } catch (error) { process.stderr.write(`Contractor reviewer-dates local qualification refused: ${error.message}\n`); process.exitCode = 2; }
}
