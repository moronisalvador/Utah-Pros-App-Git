/**
 * ════════════════════════════════════════════════
 * FILE: qualify-appointment-crew-repair-local.mjs
 * ════════════════════════════════════════════════
 *
 * WHAT THIS DOES (plain language):
 *   Qualifies the appointment-crew regression repair on two fresh, disposable,
 *   loopback-only Supabase stacks. It never links to, pushes to, or resets a
 *   hosted project. The two predecessor lineages mirror Production (the applied
 *   enum hotfix) and QA (the notification M1/M2 ledger).
 *
 * DEPENDS ON:
...
const CACHE_ROOT = path.join(os.homedir(), '.cache', 'upr-appointment-crew-repair-local');
...
export const SUCCESSOR_INPUTS = Object.freeze([
  ...
  ['supabase/tests/appointment_crew_atomic_save_and_audit_repair.test.sql', 'e25a00d792a5b230ab367a5f6cb354b9e89d7c18bf6ac61c99145c336ed575b6'],
]);
...
export const QUALIFICATION_REPOSITORY_INPUTS = Object.freeze([
  'package.json',
  'package-lock.json',
  'scripts/qa/qualify-appointment-crew-repair-local.mjs',
  'scripts/qa/safe-child-env.mjs',
  'tests/qa/unit/appointment-crew-local-qualification.test.js',
...
function createNetwork(context) {
  const name = `upr-crew-repair-${process.pid}-${randomUUID().slice(0, 8)}`;
...
DO $$ BEGIN
  IF has_table_privilege('authenticated', 'public.appointments', 'INSERT,UPDATE,DELETE') THEN RAISE EXCEPTION 'rollback retained appointment DML'; END IF;
  IF has_table_privilege('authenticated', 'public.appointment_crew', 'INSERT,UPDATE,DELETE') THEN RAISE EXCEPTION 'rollback retained crew DML'; END IF;
  IF EXISTS (
    SELECT 1
    FROM pg_attribute column_record
    CROSS JOIN (VALUES ('anon'), ('authenticated'), ('service_role')) AS role_record(role_name)
  ...
  ) THEN RAISE EXCEPTION 'rollback retained appointment column write'; END IF;
  IF EXISTS (
    SELECT 1
    FROM pg_attribute column_record
    CROSS JOIN (VALUES ('anon'), ('authenticated'), ('service_role')) AS role_record(role_name)
  ...
  ) THEN RAISE EXCEPTION 'rollback retained crew column write'; END IF;
  IF has_function_privilege('authenticated', 'public.sync_appointment_crew(uuid,jsonb)'::regprocedure, 'EXECUTE') THEN RAISE EXCEPTION 'rollback retained sync execute'; END IF;
  IF has_function_privilege('service_role', 'public.sync_appointment_crew(uuid,jsonb,uuid)'::regprocedure, 'EXECUTE') THEN RAISE EXCEPTION 'rollback retained service sync execute'; END IF;
  IF has_function_privilege('authenticated', 'public.create_appointment_with_crew(text,date,uuid,text,time,time,text,text,text,boolean,boolean,jsonb,uuid[],jsonb,uuid)'::regprocedure, 'EXECUTE') THEN RAISE EXCEPTION 'rollback retained create execute'; END IF;
  IF has_function_privilege('authenticated', 'public.update_appointment_with_crew(uuid,date,time,time,text,text,text,text,jsonb,boolean,boolean,uuid[],boolean,boolean,boolean,uuid)'::regprocedure, 'EXECUTE') THEN RAISE EXCEPTION 'rollback retained update execute'; END IF;
  IF has_function_privilege('authenticated', 'public.update_appointment(uuid,date,time,time,text,text,text,text,uuid)'::regprocedure, 'EXECUTE') THEN RAISE EXCEPTION 'rollback retained legacy update execute'; END IF;
  IF has_function_privilege('authenticated', 'public.assign_tasks_to_appointment(uuid,uuid[])'::regprocedure, 'EXECUTE') THEN RAISE EXCEPTION 'rollback retained legacy task execute'; END IF;
  IF has_function_privilege('authenticated', 'public.delete_appointment(uuid,uuid,text)'::regprocedure, 'EXECUTE') THEN RAISE EXCEPTION 'rollback retained delete execute'; END IF;
  IF has_function_privilege('authenticated', 'public.merge_jobs(uuid,uuid)'::regprocedure, 'EXECUTE') THEN RAISE EXCEPTION 'rollback retained merge execute'; END IF;
  BEGIN PERFORM public.sync_appointment_crew('00000000-0000-0000-0000-000000000001'::uuid, '[]'::jsonb); RAISE EXCEPTION 'rollback allowed sync execution'; EXCEPTION WHEN insufficient_privilege THEN NULL; END;
END $$;
ROLLBACK;`;
...
function runCycle(context, cycle, predecessor, ports) {
  ...
  try {
    ...
    for (const [relative] of [...predecessor, ...SUCCESSOR_INPUTS]) {
    ...
    if (predecessor === QA_PREDECESSOR) psql(context, container, 'postgres', ['-f', `${CONTAINER_ROOT}/inputs/seed-notification-producer-local.sql`]);
    for (const [relative] of predecessor.slice(1)) psql(context, container, 'postgres', ['-f', `${CONTAINER_ROOT}/inputs/${path.basename(relative)}`]);
    psql(context, container, 'postgres', ['-f', `${CONTAINER_ROOT}/inputs/20260804000910_appointment_crew_atomic_save_and_audit_repair.sql`]);
    ...
    psql(context, container, 'postgres', ['--single-transaction', '-f', `${CONTAINER_ROOT}/inputs/20260804000910_appointment_crew_atomic_save_and_audit_repair.rollback.sql`]);
    failClosedProof(context, container);
    psql(context, container, 'postgres', ['-f', `${CONTAINER_ROOT}/inputs/20260804000910_appointment_crew_atomic_save_and_audit_repair.sql`]);
    psql(context, container, 'postgres', ['-f', `${CONTAINER_ROOT}/inputs/appointment_crew_atomic_save_and_audit_repair.test.sql`], { isolated: true });
    process.stdout.write(`appointment-crew local qualification cycle ${cycle} passed.\n`);
  } finally {
    const errors = [];
    if (started) {
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  try { main(); } catch (error) { process.stderr.write(`Appointment-crew local qualification refused: ${error.message}\n`); process.exitCode = 2; }
}
