...
const CACHE_ROOT = path.join(os.homedir(), '.cache', 'upr-office-financial-read-boundary-local');
const CONTAINER_ROOT = '/tmp/upr-office-financial-read-boundary-local';

const MIGRATION = 'supabase/migrations/20260807230000_office_financial_read_boundary.sql';
const ROLLBACK = 'supabase/rollbacks/20260807230000_office_financial_read_boundary.rollback.sql';
const PROOF = 'supabase/tests/office_financial_read_boundary.test.sql';
const ROLLBACK_PROOF = 'supabase/tests/office_financial_read_boundary.rollback.test.sql';
...
export function main(argv = process.argv.slice(2)) {
  ...
  process.stdout.write(`${JSON.stringify({
    schema: 'upr-office-financial-read-boundary-local-qualification-v1',
    commit_sha: commit,
