...
const CACHE_ROOT = path.join(os.homedir(), '.cache', 'upr-crm-lead-boundary-local');
const CONTAINER_ROOT = '/tmp/upr-crm-lead-boundary-local';
...
const PROOF = 'supabase/tests/crm_lead_read_boundary.test.sql';
const ROLLBACK_PROOF = 'supabase/tests/crm_lead_read_boundary.rollback.test.sql';
const BASELINE = 'db/baseline/schema.sql';

// NO PREDECESSORS — and that is a measured claim, not a convenience.
//
// db/baseline/schema.sql already carries all five bodies at the exact md5s this
// migration's drift guard pins, verified 2026-08-08 by hashing the text between
// each function's dollar-quote delimiters in the baseline and comparing it to
// the live catalog:
...
// and every table it touches (crm_orgs, nav_permissions, employee_page_access,
// pipeline_stages, lead_pipeline_stage, lead_stage_history, crm_lead_notes,
// crm_tasks, system_events) exists in the baseline too.
//
// Adding the CRM migrations that ORIGINALLY defined these functions would make
// the cycle WORSE, not safer: 20260701_crm_phase4a_lead_pipeline and
// 20260702_crm_phase0F_shared_rpc_replaces both replace move_lead_to_stage with
// an OLDER body (14a79b66… / 3e696513…), so replaying them on top of the
// baseline would qualify this boundary against a body production has not had
// for weeks — and the drift guard would abort first, which is exactly its job.
//
// If the guard ever aborts here, do NOT reach for a predecessor to make it pass.
// It means the baseline has drifted from production, and the honest fix is to
// re-measure the live hashes.
...
export function main(argv = process.argv.slice(2)) {
  ...
  process.stdout.write(`${JSON.stringify({
    schema: 'upr-crm-lead-boundary-local-qualification-v1',
    commit_sha: commit,
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  try { main(); } catch (error) { process.stderr.write(`CRM lead boundary local qualification refused: ${error.message}\n`); process.exitCode = 2; }
}
