...
### Motion (tokens in `index.css` `:root`)
```css
--dur-1: 120ms   /* micro — press, toggle, hover, chip */
--dur-2: 180ms   /* base  — menus, popovers, sheets, tooltips */
--dur-3: 260ms   /* large — slide-in panels, page/section transitions */
--ease-out:    cubic-bezier(0.32, 0.72, 0, 1)     /* ENTER — the house iOS ease-out */
--ease-in:     cubic-bezier(0.4, 0, 1, 1)          /* EXIT  — accelerate away */
--ease-spring: cubic-bezier(0.34, 1.56, 0.64, 1)   /* playful overshoot — pop/check/badge */
```
Compose them — never hardcode a duration/curve: `transition: transform var(--dur-2) var(--ease-out)`.

---

## Animation & Motion Standards

**The one rule: animate `transform` and `opacity` (and `filter`) — nothing else.** Those three
run on the GPU compositor, off the main thread, and are the *only* properties eligible for the
**120fps** path on ProMotion iPhones (iOS 26). Animating `height / width / max-height / padding /
margin / top / left / border-width` forces layout every frame → main-thread, **60fps ceiling**, and
janks on long lists. `requestAnimationFrame` is hard-capped at 60fps in iOS WKWebView — never drive
motion from a JS rAF loop; use CSS.

**Every animation ships a reduced-motion guard:**
```css
@media (prefers-reduced-motion: reduce) { .thing { transition: none; animation: none; } }
```

### Recipe A — interactive press (buttons, rows, chips) · compositor, 120fps
```css
.btn { transition: transform var(--dur-1) var(--ease-out), opacity var(--dur-1) var(--ease-out); }
.btn:active { transform: scale(0.96); opacity: 0.9; }
```

### Recipe B — popover / menu reveal (PREFERRED) · compositor, 120fps
Float the element (`position: absolute/fixed`, parent `position: relative`) and animate
`transform` + `opacity`. It **overlays** content instead of pushing it — that's the price of the
compositor path, and it's how iMessage's `+` menu behaves.
```css
.menu {
  position: absolute; bottom: 100%;              /* anchor above the trigger */
...
### Recipe C — collapse/expand that MUST push content · layout-bound, 60fps
When the element has to reflow its siblings (e.g. a composer growing), you're layout-bound —
accept 60fps and keep it cheap. Animate `max-height` + `opacity`, set `max-height` **close to the
real content height** (a big round number wastes the ease-out tail), keep the subtree small.
```css
.panel { max-height: 0; opacity: 0; overflow: hidden;
  transition: max-height var(--dur-2) var(--ease-out), opacity var(--dur-1) var(--ease-out); }
.panel.open { max-height: 160px; opacity: 1; }   /* ≈ real content height, not 500px */
```
Live example: the tech Messages `+` menu (`.tv2-msgs-actions-sheet`) — it pushes the composer up,
so it *can't* be Recipe B.

### React: to animate BOTH enter AND exit, keep the node mounted
A conditional `{open && <div/>}` can only animate enter — it's unmounted before the exit runs. Keep
it mounted and toggle a class (Recipe B/C collapse it visually):
```jsx
<div className={`menu${open ? ' open' : ''}`} aria-hidden={!open}>…</div>
```
For a true unmount-on-exit, delay the unmount until `transitionend`.

### iOS / WKWebView notes
- Transforms are fine for **animation**. The "no transforms" caution elsewhere is only about the
  keep-alive **pane show/hide** (use `display:none`, per the tech-v2 panes) — not motion.
- `will-change: transform` **sparingly** — only right before a known animation; it costs memory.
  Don't leave it on everything.
- 120fps needs a ProMotion device **and** the compositor path (Recipe A/B). Recipe C and rAF stay at
  60fps by Apple's design — fine for short discrete transitions (imperceptible), wrong for scroll/drag.

### Reuse before authoring a new `@keyframes`
Grep first — these already exist: `sheetSlideUp` (bottom sheets), `createMenuIn` (dropdowns),
`spin` (loaders), `fadeIn`, `tech-slide-in` / `tech-check-pop` (tech list rows).
