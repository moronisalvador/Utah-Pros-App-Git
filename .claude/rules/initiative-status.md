...
- **Consent model:** opt-out-only for staff 1:1 service SMS + named typed transactional notices;
  ...
  it).
- **Staging database:** Supabase branch `qa-staging` (ref `dgeptnjxruclohguenex`) — the only
  hosted DB agents may iterate against. Runbook: `docs/database/staging-branch-runbook.md`.
