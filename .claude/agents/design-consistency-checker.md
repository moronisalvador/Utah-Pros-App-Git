...
9. **Motion drift** (`.claude/rules/motion-standard.md`): a raw duration/easing literal or `@keyframes`
   where a motion token (`--motion-duration-*`/`--motion-ease-*`/`--transition-*`) exists; a new
   page-level `entering`/`requestAnimationFrame(setEntering)` transition instead of the shared View
   Transitions mechanism; a transition missing its `@media (prefers-reduced-motion: reduce)` fallback;
   animating layout-triggering properties (width/height/top/left) instead of transform/opacity.
