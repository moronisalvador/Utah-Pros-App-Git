...
export async function onRequestPost(context) {
  ...
  try {
    ...
    await sendEmail(env, {
      to:      namedAddress(signer_name, signReq.signer_email),
      ...
      attachments: [{ filename: fileName, content: pdfB64, type: 'application/pdf' }],
    }).then(r => {
      if (!r.ok) console.error('Confirmation email failed:', r.status, r.error);
    });
