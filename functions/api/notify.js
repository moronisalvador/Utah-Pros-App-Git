...
async function resolveInboundMessageAudience(db, body) {
  ...
  // message read boundary. Do not fall back to the broad role audience when a
  // scoped lookup fails: that would expose an inbound notification for a thread
  // the recipient can no longer open.
  let recipients = [];
  try {
...
export async function resolveAudience(db, typeKey, body = {}) {
  ...
  if (typeKey === 'message.inbound') {
    const scopedAudience = await resolveInboundMessageAudience(db, body);
    if (scopedAudience !== null) return scopedAudience;
  }
