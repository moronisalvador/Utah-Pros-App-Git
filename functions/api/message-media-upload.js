...
async function requireConversation(db, conversationId, cors) {
  ...
  return conversation ? null : response({ error: 'Conversation not found' }, 404, cors);
}
...
export async function onRequestPost({ request, env }) {
  ...
  if (!file || typeof file.arrayBuffer !== 'function') {
    return response({ error: 'One image file is required' }, 400, cors);
  }
  ...
  try {
    ...
    return response({ error: 'The image could not be stored', code: 'MESSAGE_MEDIA_UPLOAD_FAILED' }, 503, cors);
  }
