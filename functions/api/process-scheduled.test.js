...
describe('scheduler and manual authorization', () => {
  ...
  it('constructs the cron database client with the bounded transport', async () => {
    const env = {};
    const { db } = makeDb();
    ...
    expect(supabaseMock).toHaveBeenCalledWith(env, fetchWithTimeout);
  });

  it('denies an external owner identity before any Messages or queue effect', async () => {
    requireOwnerMock.mockResolvedValue({
      employee: {
        id: 'manual-owner',
        email: 'moroni@utah-pros.com',
