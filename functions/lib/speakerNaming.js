...
export function buildResegmentPrompt(transcriptText) {
  ...
  return `This is a transcript of a two-person phone call that was NOT split by speaker — the words run together and any "Speaker" labels are wrong. Rebuild the conversation as an ordered list of turns, deciding from context who speaks each line: the AGENT works for the company (answers the phone, greets the caller, discusses the business); the CUSTOMER is the person who called in. Use a name only if that person states their own name AS THEIR OWN (e.g. "This is <name>", "My name is <name>") — include the LAST NAME too whenever they state it, not just the first name. IMPORTANT: a name mentioned while asking FOR someone ("Is this <name>?", "Can I speak to <name>?", "Can I talk to <name>?") belongs to the person being asked for, NOT to the speaker saying it — never attribute that name to the speaker themselves. When you cannot confidently tell whose name it is, leave name null rather than guessing.
