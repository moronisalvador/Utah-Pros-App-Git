 ...
 *              functions/lib/http.js (bounded database transport),
...
export async function sendGatedSms(env, {
  ...
  try {
    const result = await sendSmsWithBackoff(env, {
      ...
      // A scheduled send must not fall back to an older environment credential
      // when the managed credential store times out after a disable/rotation.
      failClosedOnCredentialError: Boolean(deliveryAttemptId),
    });
    await logSmsConsent(
      db,
      contact,
