...
describe('scheduled-message browser operation id', () => {
  ...
  it('reuses the owner-scoped id after a Capacitor WebView restart', () => {
    ...
    clearScheduledMessageOperation(
      new Map([[firstAttempt.mapKey, firstAttempt.id]]),
      firstAttempt,
      { storage },
    );
  ...
  it('does not reuse an in-memory id after the signed-in account changes', () => {
    const values = new Map();
    const storage = {
      getItem: (key) => values.get(key) || null,
      setItem: (key, value) => values.set(key, value),
    ...
    const accountA = { owner: 'v1.abcdefghijkl', epoch: 'e1.abcde' };
    const accountB = { owner: 'v1.zxcvbnmlkjhg', epoch: 'e1.fghij' };
    const operations = new Map();
    const createId = vi.fn()
      .mockReturnValueOnce('account-a-schedule-id')
      .mockReturnValueOnce('account-b-schedule-id');

    storage.setItem(PWA_ACCOUNT_OWNER_KEY, accountA.owner);
    storage.setItem(PWA_ACCOUNT_EPOCH_KEY, accountA.epoch);
    const firstAttempt = getScheduledMessageOperation(
      operations,
      REQUEST,
      { createId, ownerLease: accountA, storage },
    );

    // The screen remains mounted, but the verified device lease has changed.
    storage.setItem(PWA_ACCOUNT_OWNER_KEY, accountB.owner);
    storage.setItem(PWA_ACCOUNT_EPOCH_KEY, accountB.epoch);
    const accountBAttempt = getScheduledMessageOperation(
      operations,
      REQUEST,
      { createId, ownerLease: accountB, storage },
    );

    expect(accountBAttempt.id).toBe('account-b-schedule-id');
    expect(accountBAttempt.id).not.toBe(firstAttempt.id);
    expect(accountBAttempt.mapKey).not.toBe(firstAttempt.mapKey);
