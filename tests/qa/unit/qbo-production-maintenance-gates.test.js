...
describe('QBO production maintenance and document-command gates', () => {
  ...
  it('keeps stable invoice commands and capability-gates every estimate command before its D2 RPC work', () => {
    expect(invoice).toContain('A UUIDv4 Idempotency-Key is required for QuickBooks invoice actions');
    expect(invoice).toContain('await requireQboProviderTraffic(env)');
    expect(invoice).toContain('await reserveQboInvoiceCommand');
