...
describe('Admin Mobile light token boundary', () => {
  ...
  it('keeps the claim route touch-safe with instant visible press feedback', () => {
    ...
    expect(css).toMatch(/\.am-invoice-claim-link:active\s*\{\s*background:\s*var\(--accent-light\);\s*\}/);
    expect(css).toMatch(/@media \(prefers-reduced-motion: reduce\)\s*\{[\s\S]*?\.am-invoice-claim-link\s*\{\s*transition:\s*none;/);
  });
});
